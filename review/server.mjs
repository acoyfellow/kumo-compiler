...
app.get('/',c=>c.html(readFileSync(resolve(root,'astro/dist/index.html'),'utf8')));
...
for(const f of frameworks)app.get(`/select/${f}`,c=>f==='react'?c.html(runtimeHtml('select','react')):c.redirect(`/library-gallery/${f}/?component=select`,302));
app.get('/select/:framework/assets/:file',c=>{const{framework,file}=c.req.param();if(framework!=='react'||!/^[-\w.]+$/.test(file))return c.notFound();const body=runtimeAsset('select','react',file);return c.body(body,200,{'Content-Type':file.endsWith('.css')?'text/css':'text/javascript'});});
