...
app.get('/select/compare', (c) => c.html(`<!doctype html><html><head><meta charset="utf-8"><title>Select runtime comparison</title><style>body{margin:0;background:#111;color:#fff;font:14px system-ui}.grid{display:grid;grid-template-columns:repeat(4,minmax(280px,1fr));gap:8px;padding:16px}.cell{border:1px solid #444}.label{padding:8px;background:#222;font-weight:700}iframe{display:block;width:100%;height:620px;border:0}</style></head><body><div class="grid">${[['React','react'],['Vue','vue'],['Svelte','svelte'],['Solid','solid']].map(([label,route]) => `<div class="cell"><div class="label">${label} runtime</div><iframe title="${label}" src="/select/${route}"></iframe></div>`).join('')}</div></body></html>`));
