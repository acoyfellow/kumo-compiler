...
for(const f of frameworks)app.get(`/select/${f}`,c=>c.html(readFileSync(resolve(root,f==='vue'?`runtime/select/vue/public-runtime/index.html`:`runtime/${f}/public-runtime/index.html`),'utf8')));
app.get('/select/:framework/assets/:file',c=>{const{framework,file}=c.req.param();if(!frameworks.has(framework)||!/^[\w.-]+$/.test(file))return c.notFound();const body=readFileSync(resolve(root,framework==='vue'?`runtime/select/vue/public-runtime/assets/${file}`:`runtime/${framework}/public-runtime/assets/${file}`));return c.body(body,200,{'Content-Type':file.endsWith('.css')?'text/css':'text/javascript'});});
...
for(const kind of ['button','dialog','popover']){
 for(const f of frameworks)app.get(`/${kind}/${f}`,c=>c.html(readFileSync(resolve(root,`deploy/${kind}/${f}/index.html`),'utf8')));
