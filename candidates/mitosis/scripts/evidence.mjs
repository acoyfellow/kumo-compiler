...
for(const component of ['Field','Tabs'])for(const framework of Object.keys(plugins)){
 ...
 try{const result=await build({root:app,configFile:false,plugins:plugins[framework],build:{outDir:path.join(app,'dist'),emptyOutDir:true},logLevel:'silent'});r.build='passed';r.assets=(result.output??[]).flatMap(x=>x.output??[]).map(x=>x.fileName);
  const dist=path.join(app,'dist');server=createServer((q,s)=>{const rel=q.url==='/'?'index.html':q.url.slice(1).split('?')[0],f=path.join(dist,rel);if(!fs.existsSync(f)){if(rel==='favicon.ico'){s.statusCode=204;s.end();return}s.statusCode=404;s.end();return}s.setHeader('content-type',f.endsWith('.js')?'text/javascript':f.endsWith('.css')?'text/css':'text/html');s.end(fs.readFileSync(f))});await new Promise(ok=>server.listen(0,'127.0.0.1',ok));
  ...
  r.runtime='passed';r.consoleStatus=r.console.some(x=>x.type==='error'||x.type==='pageerror')?'failed':'passed';r.networkStatus=r.network.every(x=>x.status<400)?'passed':'failed';r.assetsStatus=r.assets.length?'passed':'failed';r.domHtml=await page.locator('#app').innerHTML();await page.screenshot({path:path.join(app,'screenshot.png')});r.screenshot=path.relative(root,path.join(app,'screenshot.png'));
