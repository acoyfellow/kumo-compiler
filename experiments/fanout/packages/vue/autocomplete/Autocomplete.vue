...
const inputValue = props.state === 'filled' ? 'United' : ''
...
<template>
  <main data-part="root" class="p-8">
    <ComboboxRoot :collection="collection" :open="open" :inputValue="inputValue" :allowCustomValue="true" inputBehavior="none">
      ...
      <ComboboxPositioner v-if="open" class="outline-none">
        <ComboboxContent :class="contentClass">
          <div :class="listClass" role="listbox">
            <ComboboxItem v-for="item in items" :key="item" :item="item" :class="itemClass" data-kumo-component="Autocomplete" data-kumo-part="item">
