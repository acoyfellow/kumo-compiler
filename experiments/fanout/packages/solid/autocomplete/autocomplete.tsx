...
export function Autocomplete(props: AutocompleteProps) {
  return <main data-part="root" class="p-8">
    <Combobox.Root collection={collection} open={props.state === 'open'} inputValue={props.state === 'filled' ? 'United' : ''} allowCustomValue inputBehavior="none">
      <Combobox.Control class={fieldClass}>
        ...
        <Combobox.Input class={inputClass} value={props.state === 'filled' ? 'United' : ''} />
      </Combobox.Control>
      <Show when={props.state === 'open'}>
        <Combobox.Positioner class="outline-none">
          <Combobox.Content class={contentClass}>
            <div class="flex-1 min-h-0 overflow-y-auto overscroll-contain scroll-pb-2 scroll-pt-2" role="listbox">
              <For each={items}>{item => <Combobox.Item item={item} class={itemClass} data-kumo-component="Autocomplete" data-kumo-part="item">
                <Combobox.ItemText class="col-start-1" />
