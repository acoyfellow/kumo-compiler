// Native Solid dropdown-menu: Ark UI Menu (Zag behavior core) + Kumo presentation classes
// on data-part anchors. No React runtime. Mirrors the validated Vue structure: unstyled
// trigger, conditional-mount content, separator border-0 (Ark MenuSeparator is <hr>),
// data-part=content vocabulary, canonical Base UI placement.
import { Menu } from '@ark-ui/solid/menu'

export interface DropdownMenuProps { state?: string; viewport?: number }
...
export function DropdownMenu(props: DropdownMenuProps) {
  ...
  return (
    <main data-part="root" class="p-8">
      <Menu.Root open={open()} positioning={{ placement: 'bottom-start', offset: { mainAxis: 8, crossAxis: -30 } }}>
        <Menu.Trigger data-part="trigger">Menu</Menu.Trigger>
        {open() && (
          <Menu.Positioner>
            <Menu.Content data-part="content" class={contentClass}>
              <Menu.Item value="edit" class={itemClass}>Edit</Menu.Item>
              <Menu.Item value="duplicate" class={itemClass}>Duplicate</Menu.Item>
              <Menu.Separator class={sepClass} />
              <Menu.Item value="delete" class={itemClass}>Delete</Menu.Item>
            </Menu.Content>
          </Menu.Positioner>
        )}
      </Menu.Root>
