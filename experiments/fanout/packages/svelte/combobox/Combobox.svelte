<script>
  import { ComboboxRoot, ComboboxControl, ComboboxInput, ComboboxTrigger, ComboboxPositioner, ComboboxContent, ComboboxList, ComboboxItem } from '@ark-ui/svelte/combobox'
  import { createListCollection } from '@ark-ui/svelte/collection'
  let { state = 'default', viewport = 1440 } = $props()
  ...
  const value = state === 'selected' ? ['United States'] : []
  const inputValue = state === 'selected' ? '["United States"]' : '[]'
...
<main data-part="root" class="p-8">
  <ComboboxRoot {collection} {open} {value} {inputValue} {positioning}>
    ...
    {#if open}
      <ComboboxPositioner><ComboboxContent class={contentClass}><ComboboxList class={listClass}>
        {#each items as item}<ComboboxItem {item} class={itemClass}><div class="col-start-1">{item}</div></ComboboxItem>{/each}
      </ComboboxList></ComboboxContent></ComboboxPositioner>
    {/if}
  </ComboboxRoot>
</main>
