...
async function run(){
  ...
  try{const {cdp}=b;await Promise.all(['Page.enable','Runtime.enable','Accessibility.enable'].map(x=>cdp.send(x)));
    for(const state of states)for(const viewport of VIEWPORTS){await cdp.send('Emulation.setDeviceMetricsOverride',{width:viewport,height:720,deviceScaleFactor:1,mobile:false});await cdp.send('Page.navigate',{url:`${origin}/?state=${state}&viewport=${viewport}`});await sleep(180);const initial=await snapshot(cdp,evaluate);const after=await snapshot(cdp,evaluate),png=initial.png;
      const trace={schemaVersion:'kumo.visual-trace/v3',component,state,viewport,dom:initial.dom,parts:initial.parts,a11y:initial.a11y,screenshot:initial.screenshot,checkpoint:{initial:{...initial,png:undefined},after:{...after,png:undefined}},behavior:{action:null,before:{focus:initial.focus,events:initial.events},after:{focus:after.focus,events:after.events}}};
