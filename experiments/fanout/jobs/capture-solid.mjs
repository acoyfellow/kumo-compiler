#!/usr/bin/env node
// Fan-out Solid capture: builds a fanout Solid component package (Ark + Kumo classes),
// SSRs + hydrates, serves, captures via ISOLATED Chrome, writes engine:'ark' v3 traces.
// Usage: node capture-solid.mjs <component>
...
if (!component) { console.error('usage: capture-solid <component>'); process.exit(2); }
const PKG = resolve(ROOT, 'packages', 'solid', component);
const OUT = resolve(ROOT, 'outputs', 'solid', component);
...
const STATES = { 'dropdown-menu': ['closed', 'open', 'dismissed'], 'select': ['default', 'selected', 'disabled'], 'dialog': ['closed', 'open', 'dismissed'], 'popover': ['closed', 'open', 'dismissed'], 'combobox': ['default', 'open', 'selected'], 'autocomplete': ['default', 'open', 'filled'], 'command-palette': ['closed', 'open', 'results'], 'menu-bar': ['default', 'open', 'second'], 'date-picker': ['closed', 'open', 'selected'], 'date-range-picker': ['closed', 'open', 'range'], 'sidebar': ['expanded', 'collapsed', 'mobile'], 'toasty': ['single', 'multiple', 'empty'] };
...
async function run(){
  ...
  const compFile = (await readdir(PKG)).find(f=>/\.tsx$/.test(f));
  if(!compFile) throw Error('no .tsx source');
  const exportName = (await readFile(resolve(PKG,compFile),'utf8')).match(/export function (\w+)/)?.[1] || 'Component';
  await writeFile(resolve(DIST,'entry-server.tsx'),`import {renderToString} from 'solid-js/web';import {${exportName}} from '${resolve(PKG,compFile)}';export function render(props){return renderToString(()=><${exportName} state={props.state} viewport={props.viewport}/>)}`);
  // Client-render (proven pattern from visual-compiler solid capture): replace SSR markup
  // with a live client render so the component is interactive; renderToString is not
  // hydratable here. The served SSR provides first paint; client render produces the live
  // DOM that the capture inspects.
  await writeFile(resolve(DIST,'client.tsx'),`import {render} from 'solid-js/web';import {${exportName}} from '${resolve(PKG,compFile)}';const p=JSON.parse(document.querySelector('#app').dataset.props);try{globalThis.__hydrated=false;document.querySelector('#app').replaceChildren();render(()=><${exportName} state={p.state} viewport={p.viewport}/>,document.querySelector('#app'));queueMicrotask(()=>{globalThis.__hydrated=true});}catch(e){globalThis.__hydrateError=String(e)}`);
  await build({root:DIST,plugins:[solid()],logLevel:'silent',build:{outDir:resolve(DIST,'client'),emptyOutDir:true,lib:{entry:resolve(DIST,'client.tsx'),formats:['es'],fileName:'app'}}});
  await build({root:DIST,plugins:[solid({ssr:true})],logLevel:'silent',build:{outDir:resolve(DIST,'server'),emptyOutDir:true,ssr:resolve(DIST,'entry-server.tsx')}});
  ...
  const server=createServer((req,res)=>{const u=new URL(req.url,'http://localhost');if(u.pathname==='/app.js'){res.setHeader('content-type','text/javascript');return res.end(client)}if(u.pathname==='/style.css'){res.setHeader('content-type','text/css');return res.end(css)}const props={state:u.searchParams.get('state'),viewport:Number(u.searchParams.get('viewport'))||1440},ssr=renderer.render(props),enc=JSON.stringify(props).replace(/'/g,'&#39;');res.setHeader('content-type','text/html');res.end(`<!doctype html><html><head><meta charset="utf-8"><link rel="stylesheet" href="/style.css"></head><body><div id="app" data-props='${enc}'>${ssr}</div><script>globalThis.__events=[]</script><script type="module" src="/app.js"></script></body></html>`)});
  ...
  try{const {cdp}=b;await Promise.all(['Page.enable','Runtime.enable','Accessibility.enable'].map(x=>cdp.send(x)));
    for(const state of states)for(const viewport of VIEWPORTS){await cdp.send('Emulation.setDeviceMetricsOverride',{width:viewport,height:720,deviceScaleFactor:1,mobile:false});await cdp.send('Page.navigate',{url:`${origin}/?state=${state}&viewport=${viewport}`});await sleep(state==='open'?400:180);const initial=await snapshot(cdp,evaluate);const after=await snapshot(cdp,evaluate),png=initial.png;
      ...
      const provenance={schemaVersion:'kumo.native-harness-provenance/v1',target:'solid',generatedSourceDigest:sha(compFile+exportName),lowererDigest:sha('fanout-ark'),nativeCompilerDigest:sha('solid+ark'),nativeBuildDigest:buildDigest,servedHarnessDigest:buildDigest,captureDigest:sha(Buffer.concat([traceBytes,png])),traceDigest:sha(traceBytes),screenshotDigest:sha(png),afterScreenshotDigest:sha(after.png),canonicalSourceDigest:sha('ark-native-not-canonical'),generatedBuild:{digest:buildDigest,framework:'solid',compiler:'@ark-ui/solid + vite-plugin-solid'},servedHarness:{url:origin,buildDigest,ssr:true,hydrated:initial.hydrated},capture:{id:sha(traceBytes).slice(0,24),capturedAt:new Date().toISOString(),independent:true,canonicalArtifactUsed:false,driver:'CDP',buildDigest,harnessDigest:buildDigest}};
  ...
  console.log(`captured solid/${component}: ${records.length} cells, reactRuntime=${reactRuntime}, hydrated=${records.every(r=>r.hydrated)}`);
}
