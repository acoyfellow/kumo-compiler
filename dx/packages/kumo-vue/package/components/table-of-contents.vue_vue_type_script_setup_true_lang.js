import { createElementBlock as e, createElementVNode as t, defineComponent as n, normalizeClass as r, openBlock as i, ref as a, renderSlot as o } from "vue";
//#region generated/libraries/vue/components/table-of-contents.vue?vue&type=script&setup=true&lang.ts
var s = { "data-kumo-part": "root" }, c = { "data-kumo-part": "collection" }, l = /*@__PURE__*/ n({
	__name: "table-of-contents",
	props: {
		Group: {},
		Item: { type: Boolean },
		List: {},
		root: {},
		Title: {}
	},
	emits: ["change"],
	setup(n, { emit: l }) {
		let u = { root: "kumo-table-of-contents-root" };
		return a(void 0), (n, a) => (i(), e("div", {
			"data-kumo-compound": "table-of-contents",
			class: r(u.root)
		}, [t("section", s, [o(n.$slots, "root")]), t("section", c, [o(n.$slots, "collection")])], 2));
	}
});
//#endregion
export { l as default };
