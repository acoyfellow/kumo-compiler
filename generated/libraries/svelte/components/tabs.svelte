...
<div {...rest} role="tablist" class="relative flex min-w-0 shrink items-stretch kumo-tabs-list overflow-x-auto rounded-lg bg-kumo-recessed px-0.5 [--scroll-fade-width:3rem] scroll-px-(--scroll-fade-width) h-9">{#each tabItems as item, index (item.value)}<button bind:this={tabButtons[index]} type="button" role="tab" class="relative z-2 flex items-center rounded bg-transparent whitespace-nowrap focus:outline-none focus:ring-kumo-focus/50 focus-visible:ring-2 focus-visible:ring-kumo-brand cursor-pointer text-base my-0.5 text-kumo-subtle hover:text-kumo-default aria-selected:text-kumo-default focus-visible:ring-inset px-2.5 rounded-md" aria-selected={selectedTabValue === item.value} tabindex={focusedIndex === index ? 0 : -1} onclickcapture={() => commitTab(index)} onkeydowncapture={(event) => handleTabKey(event, index)}>{#if selectedTabValue === item.value}<span aria-hidden="true" class="pointer-events-none absolute inset-0 -z-10 rounded-md bg-kumo-base shadow-sm ring ring-kumo-line"></span>{/if}{item.label}</button>{/each}</div>
