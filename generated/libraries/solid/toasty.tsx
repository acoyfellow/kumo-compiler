...
export function Toasty(incoming: ToastyProps): JSX.Element {
  ...
  return (<ToastManagerContext.Provider value={manager}>{props.children}<Show when={toastMounted()} children={<div tabindex="-1" role="region" aria-live="polite" aria-atomic="false" aria-relevant="additions text" aria-label="Notifications" data-kumo-component="Toasty" class="fixed top-auto right-4 bottom-4 z-1 mx-auto flex w-[calc(100%-2rem)] sm:right-8 sm:bottom-8 sm:w-[340px]"><For each={toasts()} children={toast => <Toast toast={toast} defaultVariant={props.variant as KumoToastVariant} onClose={id => manager.close(id)} onAction={props.onAction as (() => void) | undefined} />} /></div>} /></ToastManagerContext.Provider>);
}
