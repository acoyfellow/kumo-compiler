...
export function Meter(incoming: MeterProps): JSX.Element {
  ...
  return (<div class="flex w-full flex-col gap-2" role="meter" aria-valuenow={props.value as number} aria-valuemin={(props.min as number) ?? 0} aria-valuemax={(props.max as number) ?? 100} aria-valuetext={Math.round((((props.value as number) ?? 0) - ((props.min as number) ?? 0)) / (((((props.max as number) ?? 100) - ((props.min as number) ?? 0)) as number) || 1) * 100) + "%"}><div class="flex items-center justify-between gap-4"><span role="presentation" class="text-xs text-kumo-subtle">{props.label as JSX.Element}</span>{(props.showValue as boolean) !== false ? <span aria-hidden="true" class="text-sm font-medium text-kumo-default tabular-nums">{(props.customValue as JSX.Element) ?? ((props.value as number) + "%")}</span> : null}</div><div class="relative h-2 w-full overflow-hidden rounded-full bg-kumo-fill"><div class="absolute inset-y-0 left-0 rounded-full bg-linear-to-r from-kumo-brand via-kumo-brand to-kumo-brand transition-[width] duration-300 ease-out" style={{ "inset-inline-start": "0", height: "inherit", width: (props.value as number) + "%" }} /></div><span role="presentation" style="clip-path:inset(50%);overflow:hidden;white-space:nowrap;border:0;padding:0;width:1px;height:1px;margin:-1px;position:fixed;top:0;left:0">x</span></div>);
}
