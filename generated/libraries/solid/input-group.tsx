...
export function InputGroup(incoming: InputGroupProps): JSX.Element {
  ...
  return (<div data-kumo-component="InputGroup"><label for={inputGroupId}>{inputGroupFixture().props.label as JSX.Element}</label><span>{inputGroupFixture().props.description as JSX.Element}</span>{compoundFixtureText(props.fixture, ".Addon")}<input id={inputGroupId} aria-label={inputGroupPart(".Input").props["aria-label"] as string} disabled={Boolean(inputGroupPart(".Input").props.disabled)} value={inputGroupValue()} onInput={inputGroupOnInput} /><button type="button" data-variant={inputGroupPart(".Button").props.variant as string}>{compoundFixtureText(props.fixture, ".Button")}</button>{compoundFixtureText(props.fixture, ".Suffix")}</div>);
}
