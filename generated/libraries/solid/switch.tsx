...
export function Switch(incoming: SwitchProps): JSX.Element {
  ...
  return ({props.label !== undefined ? (<label class="inline-flex items-center gap-2 cursor-pointer select-none text-base text-kumo-default"><button class={mergeStyles("relative inline-flex items-center h-4 w-8 rounded-[5px] ring ring-kumo-hairline cursor-pointer border-none p-0 focus:outline-none focus:ring-kumo-focus/50 focus-visible:ring-2 focus-visible:ring-kumo-brand transition-colors duration-150 ease-out", (props.size === "sm") ? "h-4 w-8" : "", (props.size === "lg") ? "h-5 w-10" : "")} data-state={currentIndeterminate() ? "indeterminate" : (checked() ? "checked" : "unchecked")} data-checked={checked() ? "" : undefined} data-unchecked={checked() ? undefined : ""} data-indeterminate={currentIndeterminate() ? "" : undefined} aria-label={props["aria-label"] as string} type="button" role="switch" aria-checked={checked()} disabled={Boolean(props.disabled)} onClick={toggleChecked}><span aria-hidden="true" class="absolute top-0.5 left-0.5 h-3 w-3 rounded-[4px] bg-kumo-base shadow-[0_0_1px_0.5px_var(--color-kumo-shadow-edge),0_1px_2px_var(--color-kumo-shadow-drop)] transition-all duration-150 ease-out" /></button><span>{props.label as string}</span></label>) : (<button class={mergeStyles("relative inline-flex items-center h-4 w-8 rounded-[5px] ring ring-kumo-hairline cursor-pointer border-none p-0 focus:outline-none focus:ring-kumo-focus/50 focus-visible:ring-2 focus-visible:ring-kumo-brand transition-colors duration-150 ease-out", (props.size === "sm") ? "h-4 w-8" : "", (props.size === "lg") ? "h-5 w-10" : "")} data-state={currentIndeterminate() ? "indeterminate" : (checked() ? "checked" : "unchecked")} data-checked={checked() ? "" : undefined} data-unchecked={checked() ? undefined : ""} data-indeterminate={currentIndeterminate() ? "" : undefined} aria-label={props["aria-label"] as string} type="button" role="switch" aria-checked={checked()} disabled={Boolean(props.disabled)} onClick={toggleChecked}><span aria-hidden="true" class="absolute top-0.5 left-0.5 h-3 w-3 rounded-[4px] bg-kumo-base shadow-[0_0_1px_0.5px_var(--color-kumo-shadow-edge),0_1px_2px_var(--color-kumo-shadow-drop)] transition-all duration-150 ease-out" /></button>)});
}
