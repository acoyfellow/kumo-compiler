...
<template>
  <div class="flex w-full flex-col gap-2" role="meter" :aria-valuenow="props.value" :aria-valuemin="props.min ?? 0" :aria-valuemax="props.max ?? 100" :aria-valuetext="Math.round(((props.value ?? 0) - (props.min ?? 0)) / (((props.max ?? 100) - (props.min ?? 0)) || 1) * 100) + '%'"><div class="flex items-center justify-between gap-4"><span class="text-xs text-kumo-subtle">{{ props.label }}</span><span v-if="props.showValue !== false" class="text-sm font-medium text-kumo-default tabular-nums">{{ props.customValue ?? (props.value + '%') }}</span></div><div class="relative h-2 w-full overflow-hidden rounded-full bg-kumo-fill"><div class="absolute inset-y-0 left-0 rounded-full bg-linear-to-r from-kumo-brand via-kumo-brand to-kumo-brand transition-[width] duration-300 ease-out" :style="{ width: props.value + '%' }"></div></div><span role="presentation" style="clip-path:inset(50%);overflow:hidden;white-space:nowrap;border:0;padding:0;width:1px;height:1px;margin:-1px;position:fixed;top:0;left:0">x</span></div>
</template>
