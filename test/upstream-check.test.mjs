 ...
 try {
  for(const scenario of ['real','synthetic-export-break']){
   ...
   try {
    ...
    const deterministic=v=>({...v,generation:{...v.generation,browser:undefined},receiptSha256:undefined});assert.deepEqual(deterministic(x),deterministic(y));assert.equal(x.schemaVersion,'kumo.upstream.receipt/v3');for(const side of ['baseline','candidate'])for(const field of ['exports','declarations','tokens','files']){assert(Number.isSafeInteger(x.inventories[side][field].count));assert.match(x.inventories[side][field].sha256,/^[a-f0-9]{64}$/)}assert.match(x.source.sourceTree,/^[a-f0-9]{64}$/);assert.equal(x.receiptSha256,sha(stable(Object.fromEntries(Object.entries(x).filter(([k])=>k!=='receiptSha256')))));assert.equal(x.authority.selectedAuthorityMutated,false);assert.equal(x.generation.componentId,'button');assert.equal(x.generation.cells.length,4);assert(!JSON.stringify(x).includes('/tmp/'));
    assert(Number.isSafeInteger(x.generation.browser.passedCount));assert(x.generation.browser.passedCount>0,'system Chrome must pass at least one generated framework');assert.equal(x.generation.browser.cells.length,4);for(const cell of x.generation.browser.cells){assert(cell.generatedImportPaths.every(Boolean));if(cell.status==='blocked')assert(cell.diagnostics.some(d=>d.length>40&&!/generated source API could not/.test(d)))}
