...
test('pure validator rejects mutations and every nonterminal gate status',()=>{
 ...
 const stale=fixture();stale.production.identity.commit='e'.repeat(40);assert.equal(validateTerminalInputs(stale).valid,false);
...
test('repository invocation fails closed while production/canonical terminal prerequisites are absent',()=>{
 const result=spawnSync(process.execPath,['scripts/build-terminal-receipt.mjs'],{encoding:'utf8'});
 assert.notEqual(result.status,0);
 assert.match(result.stderr,/terminal prerequisite (unavailable|rejected)/);
});
