...
test('mitosis matrix has complete bound identities and non-optimistic statuses',()=>{
  ...
  for(const r of matrix.rows){
    ...
    assert.equal(r.candidateRevision,'d1bcccf'); assert.equal(r.runId,'ter_20260620185149117_v1naq0');
    ...
    if(r.component==='Button') assert.equal(r.status,'passed');
  ...
  assert.deepEqual(matrix.counts,gates); assert.deepEqual(matrix.targetCounts,targets); assert.deepEqual(targets,{passed:4,partial:24,failed:0,blocked:0}); assert.equal(matrix.adaptationFrequency,28);
});
