...
for(const asset of manifest.assets){const bytes=await readFile(resolve(dir,asset.file));if(bytes.length!==asset.bytes||sha(bytes)!==asset.sha256)throw Error(`${asset.file}: bytes/hash mismatch`);if(!asset.githubUrl.includes(`/releases/download/${manifest.tag}/`))throw Error(`${asset.file}: invalid GitHub URL`);if(!asset.package.startsWith('@cloudflare/kumo-')||asset.components.join(',')!=='Button,Field')throw Error(`${asset.file}: package contract`)}
