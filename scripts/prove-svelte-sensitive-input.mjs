...
const T=fs.mkdtempSync(path.join(os.tmpdir(),'sisv-'));const app=path.join(T,'consumer');fs.mkdirSync(path.join(app,'src'),{recursive:true});
...
function appSource(plan){const props=canonicalProps(plan.vector);const seed=SEED[plan.vector]||[];return `<script>import SensitiveInput from'@acoyfellow/kumo-svelte/sensitive-input';const props=${JSON.stringify(props)};let events=$state([]);let clip=$state(${JSON.stringify(seed)});let live=$state([]);if(typeof window!=='undefined'){globalThis.__si={get events(){return events},get clip(){return clip},get live(){return live}};try{Object.defineProperty(navigator,'clipboard',{configurable:true,value:{writeText:async t=>{clip=[...clip,t]}}})}catch(e){}}function onValueChange(v){events=[...events,'value:'+v]}function onCopy(){events=[...events,'copy'];const r=document.querySelector('#v0 [aria-live]');const m=document.querySelector('#v0 [data-kumo-part=masked-container]');if(r)live=[...live,(m?m.textContent.trim():'')+r.textContent.trim()]}</script><section id="v0"><SensitiveInput {...props} {onValueChange} {onCopy}/></section>`;}
...
console.log('svelte sensitive-input proven:',cells.length,'cells; counts',json(receipt.counts));
