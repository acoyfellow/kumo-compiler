...
export async function validateAndFanIn({runId,shardFiles,evidenceRoot,workspaceRoot}){
 workspaceRoot=workspaceRoot||evidenceRoot;
 const expected=new Map(targets.map(t=>[`${t.component}/${t.framework}`,t])),seen=new Map();
 for(const file of [...shardFiles].sort()){
  const shard=JSON.parse(await readFile(file,'utf8'));if(shard.runId!==runId)throw Error(`shard run identity mismatch: ${file}`);if(shard.status!=='passed')throw Error(`failed shard: ${file}`);if(!frameworks.includes(shard.framework))throw Error(`invalid shard framework: ${file}`);if(!Array.isArray(shard.components)||!shard.components.length||new Set(shard.components).size!==shard.components.length||!Array.isArray(shard.outputRoots)||shard.outputRoots.length!==shard.components.length||shard.outputRoots.some((p,i)=>!inside(workspaceRoot,p)||!resolve(p).includes(`/${shard.components[i]}/`)))throw Error(`invalid shard manifest: ${file}`);if((shard.results||[]).some(r=>r.framework!==shard.framework||!shard.components.includes(r.component)))throw Error(`mixed framework identities: ${file}`);
...
export async function promoteAtomic({summary,authorityRoot,evidenceRoot,lockPath=resolve(authorityRoot,'.promotion.lock'),simulateInterrupt=false}){
 await mkdir(authorityRoot,{recursive:true});let lock,temp,stage;try{lock=await open(lockPath,'wx');let promoted=summary;if(evidenceRoot){const runs=resolve(authorityRoot,'matrix-runs');await mkdir(runs,{recursive:true});stage=resolve(runs,`.${summary.runId}.${randomUUID()}.tmp`);for(const result of summary.results){const sourceDir=dirname(result.evidence),destDir=resolve(stage,'evidence',relative(evidenceRoot,sourceDir));if(!inside(evidenceRoot,result.evidence))throw Error('promotion evidence escapes root');await mkdir(dirname(destDir),{recursive:true});await cp(sourceDir,destDir,{recursive:true,errorOnExist:true})}const finalRun=resolve(runs,summary.runId);await rename(stage,finalRun);stage=null;promoted={...summary,results:summary.results.map(r=>({...r,evidence:resolve(finalRun,'evidence',relative(evidenceRoot,r.evidence))}))}}
