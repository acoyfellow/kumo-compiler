...
export async function buildStagedComponent({root,dir,out,framework,component,runCommand}){
 ...
 else {const markup=typeof rendered==='string'?rendered:rendered.body,id=framework==='react'?'root':'app',open=`<div id="${id}">`,start=html.indexOf(open);if(start<0)throw Error(`${framework}/${component}: SSR mount missing`);let depth=1,cursor=start+open.length,end=-1;const tag=/<\/?div\b[^>]*>/g;tag.lastIndex=cursor;for(let match;(match=tag.exec(html));){depth+=match[0][1]==='/'?-1:1;if(depth===0){end=match.index+match[0].length;break}}if(end<0)throw Error(`${framework}/${component}: SSR mount is unbalanced`);html=html.slice(0,start)+`${open}${markup}</div>`+html.slice(end)}
