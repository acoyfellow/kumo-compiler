...
const T=fs.mkdtempSync(path.join(os.tmpdir(),'dlgso-'));const app=path.join(T,'consumer');fs.mkdirSync(path.join(app,'src'),{recursive:true});
...
function bodySrc(plan){const fixture=canonicalFixture(plan.vector);return `import{createSignal}from'solid-js';import{Dialog}from'@acoyfellow/kumo-solid/dialog';const fixture=${JSON.stringify(fixture)};const[events,setEvents]=createSignal([]);if(typeof window!=='undefined')globalThis.__dlg={get events(){return events()}};function App(){return <section id="v0"><Dialog fixture={fixture} onOpenChange={(v)=>setEvents([...events(),'open:'+v])}/></section>}`;}
function sources(plan){return{ssr:`import{generateHydrationScript,renderToString}from'solid-js/web';${bodySrc(plan)}console.log(JSON.stringify({html:renderToString(App,{renderId:'kumo-'}),hs:generateHydrationScript()}));`,client:`import{hydrate}from'solid-js/web';${bodySrc(plan)}const before=document.querySelector('#v0 > button');hydrate(App,document.querySelector('#app'),{renderId:'kumo-'});queueMicrotask(()=>{globalThis.__nodePreserved=before===document.querySelector('#v0 > button');globalThis.__ready=true});`};}
...
console.log('solid dialog proven:',cells.length,'cells; counts',json(receipt.counts));
