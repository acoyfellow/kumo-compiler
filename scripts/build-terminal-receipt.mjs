...
export function validateTerminalInputs(input){
 ...
 assertCount(errors,kitchenSink?.readiness==='41/41'&&Array.isArray(kitchenSink?.renderedComponents)&&kitchenSink.renderedComponents.length===41&&['vue','svelte','solid'].every(f=>SHA.test(kitchenSink?.packages?.[f]?.sha256??''))&&Array.isArray(kitchenSink?.imports)&&kitchenSink.imports.length===123&&Array.isArray(kitchenSink?.omissions)&&kitchenSink.omissions.length===0,'kitchen-sink must be the all-41 package-backed catalog with 123 imports and no omissions');
 ...
 assertCount(errors,typeof currentCommit==='string'&&/^[0-9a-f]{40}$/.test(currentCommit)&&/^[0-9a-f]{40}$/.test(production?.identity?.commit??''),'terminal and deployed commits must be exact git SHAs');
...
export async function buildTerminalReceipt({repoRoot=root}={}){
 ...
 for(const [framework,selection] of Object.entries(loaded.browserAuthority.value.selected??{})){const bytes=await readFile(resolve(repoRoot,selection.manifest));loaded[`browser-${framework}`]={path:selection.manifest,value:JSON.parse(bytes),sha256:sha256Bytes(bytes)}}
 const currentCommit=execFileSync('git',['rev-parse','HEAD'],{cwd:repoRoot,encoding:'utf8'}).trim();
 const vectorCount=loaded.observable.value.vectorCount, contracted=loaded.observable.value.counts?.contracted;
 const browserRun=framework=>loaded[`browser-${framework}`].value;
 const frameworkSummary=framework=>({status:'passed',vectorsPassed:vectorCount,vectorsTotal:vectorCount,components:browserRun(framework).components?.length});
 const input={authority:loaded.authority.value,canonical:{status:'passed',contracted,vectorsPassed:vectorCount,vectorsTotal:vectorCount,browserCells:Object.keys(loaded.browserAuthority.value.selected??{}).length*contracted,immutable:true},frameworks:{vue:frameworkSummary('vue'),svelte:frameworkSummary('svelte'),solid:frameworkSummary('solid')},readiness:loaded.readiness.value,examples:loaded.examples.value,docs:loaded.docs.value,kitchenSink:loaded.kitchenSink.value,release:loaded.release.value,production:loaded.production.value,progress:loaded.progress.value,currentCommit};
