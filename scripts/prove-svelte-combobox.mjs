...
const T=fs.mkdtempSync(path.join(os.tmpdir(),'cbsv-'));const app=path.join(T,'consumer');fs.mkdirSync(path.join(app,'src'),{recursive:true});
...
function appSource(plan){const fixture=canonicalFixture(plan.vector);return `<script>import Combobox from'@acoyfellow/kumo-svelte/combobox';const fixture=${JSON.stringify(fixture)};let events=$state([]);if(typeof window!=='undefined')globalThis.__cb={get events(){return events}};</script><section id="v0"><Combobox {fixture} onOpenChange={(v)=>{events=[...events,'open:'+v]}} onValueChange={(v)=>{events=[...events,'value:'+v]}}/></section>`;}
...
console.log('svelte combobox proven:',cells.length,'cells; counts',json(receipt.counts));
