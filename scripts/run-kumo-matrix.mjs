...
try{await Promise.all(Array.from({length:Math.min(limit,groups.length)},launch));if(failed)throw failed;const summary=await validateAndFanIn({runId,shardFiles:files,evidenceRoot:workspace,workspaceRoot:workspace});await immutableJson(resolve(workspace,'matrix-summary.json'),summary);await promoteAtomic({summary,authorityRoot,evidenceRoot:workspace});console.log(`kumo matrix passed: ${summary.targetCount} targets, run ${runId}`)}catch(e){failed=e;for(const c of children)c.kill('SIGTERM');throw e}finally{if(!arg('keep-temp'))await rm(workspace,{recursive:true,force:true})}
