...
if(locked?.version!=='2.5.2'||!locked.resolved||!locked.integrity)throw new Error('npm lock does not bind Kumo 2.5.2 with URL and integrity');
...
await mkdir(resolve(root,'audit'),{recursive:true});await writeFile(resolve(root,'audit/kumo-react-2.5.2.provenance.json'),JSON.stringify(manifest,null,2)+'\n');
