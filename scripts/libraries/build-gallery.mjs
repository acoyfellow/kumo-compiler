...
const fixtures = {
  vue: `import { createApp, h, ref } from 'vue'; import { Button } from '@acoyfellow/kumo-vue/button'; import { Field } from '@acoyfellow/kumo-vue/field'; import '@acoyfellow/kumo-vue/styles.css';
const App={setup(){const clicks=ref(0),value=ref('Package field');return()=>h('main',{},[h('section',{},[h('h2',{},'Button'),h(Button,{onClick:()=>clicks.value++},()=>\`Clicked \${clicks.value}\`),h(Button,{disabled:true},()=>\`Disabled\`)]),h('section',{},[h('h2',{},'Field'),h(Field,{id:'gallery-field',label:'Project name',description:'Rendered by the installed package.',error:'Example validation message.',required:true,modelValue:value.value,'onUpdate:modelValue':v=>value.value=v}),h('output',{'aria-live':'polite'},\`Model: \${value.value}\`)])])}}; createApp(App).mount('#app');`,
  svelte: `import { mount } from 'svelte'; import App from './App.svelte'; import '@acoyfellow/kumo-svelte/styles.css'; mount(App,{target:document.getElementById('app')});`,
  solid: `import { render } from 'solid-js/web'; import { createSignal } from 'solid-js'; import { Button } from '@acoyfellow/kumo-solid/button'; import { Field } from '@acoyfellow/kumo-solid/field'; import '@acoyfellow/kumo-solid/styles.css';
