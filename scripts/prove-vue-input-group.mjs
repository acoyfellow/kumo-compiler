...
const T=fs.mkdtempSync(path.join(os.tmpdir(),'igv-'));const app=path.join(T,'consumer');fs.mkdirSync(path.join(app,'src'),{recursive:true});
...
function sources(plan){
 ...
 const body=`const fixture=${json(fixture)};const App={setup(){return()=>h('section',{id:'v0'},[h(InputGroup,{fixture})])}};`;
 return{ssr:`import{createSSRApp,h}from'vue';import{renderToString}from'@vue/server-renderer';import{InputGroup}from'@acoyfellow/kumo-vue/input-group';${body}console.log(JSON.stringify(await renderToString(createSSRApp(App))));`,
 client:`import{createSSRApp,h,nextTick}from'vue';import{InputGroup}from'@acoyfellow/kumo-vue/input-group';${body}const before=document.querySelector('#v0 > div');const a=createSSRApp(App);a.mount('#app');nextTick(()=>{globalThis.__nodePreserved=before===document.querySelector('#v0 > div');globalThis.__ready=true});`};
}
...
console.log('vue input-group proven:',cells.length,'cells; counts',json(receipt.counts));
