...
const T=fs.mkdtempSync(path.join(os.tmpdir(),'cbv-'));const app=path.join(T,'consumer');fs.mkdirSync(path.join(app,'src'),{recursive:true});
...
function sources(plan){
 ...
 const body=`const fixture=${json(fixture)};const events=ref([]);if(typeof window!=='undefined')globalThis.__cb={events};const App={setup(){return()=>h('section',{id:'v0'},[h(Combobox,{fixture,onOpenChange:v=>{events.value=[...events.value,'open:'+v]},onValueChange:v=>{events.value=[...events.value,'value:'+v]}})])}};`;
 return{ssr:`import{createSSRApp,h,ref}from'vue';import{renderToString}from'@vue/server-renderer';import{Combobox}from'@acoyfellow/kumo-vue/combobox';${body}console.log(JSON.stringify(await renderToString(createSSRApp(App))));`,
 client:`import{createSSRApp,h,ref,nextTick}from'vue';import{Combobox}from'@acoyfellow/kumo-vue/combobox';${body}const before=document.querySelector('#v0 input');const a=createSSRApp(App);a.mount('#app');nextTick(()=>{globalThis.__nodePreserved=before===document.querySelector('#v0 input');globalThis.__ready=true});`};
}
const probeFn=pr=>`(()=>{const box=document.querySelector('#v0'),input=box.querySelector('input'),active=document.activeElement,kind=${json(pr.kind)};if(kind==='dom'){return {tag:input.tagName.toLowerCase()};}if(kind==='state'){return {value:input.value};}if(kind==='events'){return [...(globalThis.__cb?globalThis.__cb.events.value:[])];}if(kind==='focus'){return active===input?'input':(active&&active.tagName==='BUTTON'?'button':'none');}if(kind==='node-identity'){return globalThis.__nodePreserved?'preserved':'replaced';}})()`;
const cells=[];
...
console.log('vue combobox proven:',cells.length,'cells; counts',json(receipt.counts));
