...
const T=fs.mkdtempSync(path.join(os.tmpdir(),'dlgsv-'));const app=path.join(T,'consumer');fs.mkdirSync(path.join(app,'src'),{recursive:true});
...
function appSource(plan){const fixture=canonicalFixture(plan.vector);return `<script>import Dialog from'@acoyfellow/kumo-svelte/dialog';const fixture=${JSON.stringify(fixture)};let events=$state([]);if(typeof window!=='undefined')globalThis.__dlg={get events(){return events}};</script><section id="v0"><Dialog {fixture} onOpenChange={(v)=>{events=[...events,'open:'+v]}}/></section>`;}
...
console.log('svelte dialog proven:',cells.length,'cells; counts',json(receipt.counts));
