...
const T=fs.mkdtempSync(path.join(os.tmpdir(),'acso-'));const app=path.join(T,'consumer');fs.mkdirSync(path.join(app,'src'),{recursive:true});
...
function bodySrc(plan){const fixture=canonicalFixture(plan.vector);return `import{createSignal}from'solid-js';import{Autocomplete}from'@acoyfellow/kumo-solid/autocomplete';const fixture=${JSON.stringify(fixture)};const[events,setEvents]=createSignal([]);if(typeof window!=='undefined')globalThis.__ac={get events(){return events()}};function App(){return <section id="v0"><Autocomplete fixture={fixture} onOpenChange={(v)=>setEvents([...events(),'open:'+v])} onValueChange={(v)=>setEvents([...events(),'value:'+v])}/></section>}`;}
...
console.log('solid autocomplete proven:',cells.length,'cells; counts',json(receipt.counts));
