...
`index.mjs` validates inventory, provenance, stable SHA-256 digests, the closed capability taxonomy, readiness, and the anti-lookalike boundary. `algebra.mjs` defines the strict framework-neutral expression, node, and operation grammar. `capabilities/compound-exports.json` is a versioned, content-addressed graph derived from every shared packed fixture reference and its owning contract public exports; models carry the corresponding path trees and fixture vector provenance. Component roots are implementation data only; contract fixtures remain external.
