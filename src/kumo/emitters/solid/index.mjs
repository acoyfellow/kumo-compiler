...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls, radioGroup, tabsNavigation, menubarNavigation, dialogLayer, inputGroupComposition, sensitiveInput, comboboxCollection, autocompleteCollection, commandPalette, toastLifecycle, datePicker, dateRangePicker, responsiveSidebar, popoverLayer, dropdownMenuLayer, selectCollection) {
  ...
  const dropdownFallback = dropdownMenuLayer ? `<><Show when={dropdownOpen()} children={<span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span>} /><button ref={dropdownTrigger} type="button" tabindex="0" aria-haspopup="menu" aria-expanded={dropdownOpen()} data-kumo-component={dropdownOpen() ? "Button" : undefined} class={dropdownOpen() ? ${JSON.stringify(KUMO_OVERLAY_BUTTON_CLASS)} : undefined} onClick={toggleDropdownFromPointer} onKeyDown={dropdownTriggerKeyDown}>{dropdownTriggerText()}</button><Show when={dropdownOpen()} children={<><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span><div data-base-ui-portal=""><div role="presentation" style="position:absolute;left:0;top:0;transform:translate(-19px,44px)"><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span><div ref={dropdownMenu} role="menu" class=${JSON.stringify(KUMO_DROPDOWN_CONTENT_CLASS)}><For each={dropdownItems()} children={item => <div role="menuitem" tabindex="-1" aria-disabled={item.disabled || undefined} data-kumo-component="DropdownMenu" data-kumo-part="item" data-disabled={item.disabled ? "" : undefined} class=${JSON.stringify(KUMO_DROPDOWN_ITEM_CLASS)} onClick={event => selectDropdownItem(item, event)} onKeyDown={event => dropdownItemKeyDown(event, item)}>{item.text}</div>} /></div><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span></div></div><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span><span style="clip-path:inset(50%);position:fixed;top:0;left:0"></span><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span></>} /></>` : null;
  ...
  const comboboxFallback = comboboxCollection ? `<><input ref={comboboxInput} role="combobox" aria-expanded={comboboxOpen()} aria-haspopup="listbox" aria-autocomplete="list" placeholder={comboboxTrigger().props.placeholder as string} style=${JSON.stringify(KUMO_CHECKBOX_HIDDEN_INPUT_STYLE)} value={comboboxQuery()} onClick={() => setComboboxOpen(true)} onInput={comboboxOnInput} onKeyDown={comboboxKeyDown} /><Show when={comboboxOpen() && filteredComboboxItems().length > 0} children={<ul role="listbox"><For each={filteredComboboxItems()} children={(item, index) => <li role="option" tabindex="-1" data-value={item.value} aria-selected={highlightedIndex() === index()} aria-disabled={Boolean(item.props.disabled) || undefined} onPointerDown={event => selectComboboxItem(item, event)}>{fixtureText(item)}</li>} /></ul>} /></>` : null;
