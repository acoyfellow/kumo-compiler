...
function declaration(model, toggle, nativeInput, clipboardCopy, paginationControls, menubarNavigation, dialogLayer, inputGroupComposition, sensitiveInput, comboboxCollection, autocompleteCollection) {
  const nativeButton=Boolean(model.interactions?.nativeButton);
  const nativeInputElement=nativeInput?.requirements.dom?.[0];
  const modelProps = (comboboxCollection || autocompleteCollection) ? [
...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls, radioGroup, tabsNavigation, menubarNavigation, dialogLayer, inputGroupComposition, sensitiveInput, comboboxCollection, autocompleteCollection) {
  ...
  const nativeInputElement=nativeInput?.requirements.dom?.[0];
  if(nativeButton||paginationControls||radioGroup||tabsNavigation||menubarNavigation||dialogLayer||inputGroupComposition||sensitiveInput||comboboxCollection||autocompleteCollection)imports.add('mergeProps');
  if(toggle || clipboardCopy || paginationControls || radioGroup || tabsNavigation || dialogLayer || inputGroupComposition || sensitiveInput || comboboxCollection || autocompleteCollection)imports.add('createSignal');
  ...
  const fallback = autocompleteFallback ?? comboboxFallback ?? sensitiveInputFallback ?? inputGroupFallback ?? dialogFallback ?? menubarFallback ?? tabsFallback ?? radioFallback ?? paginationFallback ?? clipboardFallback ?? (nativeButton
