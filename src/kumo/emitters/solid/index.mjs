...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls, radioGroup, tabsNavigation) {
  ...
  const nativeInputElement=nativeInput?.requirements.dom?.[0];
  if(nativeButton||paginationControls||radioGroup||tabsNavigation)imports.add('mergeProps');
  if(toggle || clipboardCopy || paginationControls || radioGroup || tabsNavigation)imports.add('createSignal');
  if(radioGroup || tabsNavigation)imports.add('For');
  ...
  const fallback = tabsFallback ?? radioFallback ?? paginationFallback ?? clipboardFallback ?? (nativeButton
...
export function emitSolidLibrary({libraryPath, outputPath} = {}) {
  ...
  for (const model of library.models) {
    ...
    const js = source(model, toggle, nativeInput, fieldControl, modelClipboardCopy, modelPaginationControls, modelRadioGroup, modelTabsNavigation), dts = declaration(model, toggle, nativeInput, modelClipboardCopy, modelPaginationControls), variants = model.draftImplementation.semanticVariants ?? [];
