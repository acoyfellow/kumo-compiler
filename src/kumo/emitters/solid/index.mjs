...
function declaration(model, toggle, nativeInput, clipboardCopy, paginationControls) {
  const nativeButton=Boolean(model.interactions?.nativeButton);
  const nativeInputElement=nativeInput?.requirements.dom?.[0];
  const modelProps = paginationControls ? [
  ...
  const props = modelProps.map(item => `  ${JSON.stringify(item.name)}${item.required ? '' : '?'}: ${item.type === 'number' ? 'number' : item.type === 'pagination-mode' ? '"simple" | "dropdown"' : item.type === 'pagination-labels' ? '{ navigation?: string; previousPage?: string; nextPage?: string }' : item.type === 'pagination-callback' ? '(page: number) => void' : nativeButton&&['disabled','loading'].includes(item.name)||toggle&&[toggle.controlledProp,toggle.defaultProp,toggle.disabled.prop,toggle.indeterminate?.prop].includes(item.name)?'boolean':safeType(item.type).includes('callback') ? toggle&&item.name==='onCheckedChange'?'(checked: boolean) => void':clipboardCopy&&item.name==='onCopy'?'() => void':'(...args: unknown[]) => void' : clipboardCopy&&[clipboardCopy.copySource.prop,clipboardCopy.copySource.fallback].includes(item.name)?'string':'unknown'};`).join('\n');
...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls) {
  ...
  if(toggle || clipboardCopy || paginationControls)imports.add('createSignal');
  ...
  const semantic = (nativeInputElement || clipboardCopy || paginationControls) ? '' : [...variants].sort((a, b) => b.when.length - a.when.length).map(variant => `  if (${variant.when.map(predicate).join(' && ') || 'true'}) return (${node(variant.tree)});`).join('\n');
...
` : ''}${paginationControls ? `  const maxPage = () => Math.max(1, Math.ceil((props.totalCount as number) / (props.perPage as number)));
  ...
  const commitInput = (element: HTMLInputElement) => { const value = element.value.trim(); if (!/^\\d+$/.test(value)) { setInputValue(String(currentPage())); element.value = String(currentPage()); return; } proposePage(Number.parseInt(value, 10)); };
  ...
  const pageBlur: JSX.EventHandlerUnion<HTMLInputElement, FocusEvent> = event => commitInput(event.currentTarget);
...
export function emitSolidLibrary({libraryPath, outputPath} = {}) {
  ...
  for (const model of library.models) {
    ...
    const js = source(model, toggle, nativeInput, fieldControl, modelClipboardCopy, modelPaginationControls), dts = declaration(model, toggle, nativeInput, modelClipboardCopy, modelPaginationControls), variants = model.draftImplementation.semanticVariants ?? [];
