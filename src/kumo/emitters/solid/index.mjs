...
function declaration(model, toggle, nativeInput, clipboardCopy, paginationControls, menubarNavigation, dialogLayer, inputGroupComposition, sensitiveInput, comboboxCollection, autocompleteCollection, commandPalette, toastLifecycle, datePicker, dateRangePicker, popoverLayer) {
  const nativeButton=Boolean(model.interactions?.nativeButton);
  const nativeInputElement=nativeInput?.requirements.dom?.[0];
  const modelProps = dateRangePicker ? [
...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls, radioGroup, tabsNavigation, menubarNavigation, dialogLayer, inputGroupComposition, sensitiveInput, comboboxCollection, autocompleteCollection, commandPalette, toastLifecycle, datePicker, dateRangePicker, responsiveSidebar, popoverLayer) {
  ...
  const nativeInputElement=nativeInput?.requirements.dom?.[0];
  if(nativeButton||paginationControls||radioGroup||tabsNavigation||menubarNavigation||dialogLayer||inputGroupComposition||sensitiveInput||comboboxCollection||autocompleteCollection||commandPalette||popoverLayer)imports.add('mergeProps');
  if(toggle || clipboardCopy || paginationControls || radioGroup || tabsNavigation || dialogLayer || inputGroupComposition || sensitiveInput || comboboxCollection || autocompleteCollection || commandPalette || popoverLayer)imports.add('createSignal');
  ...
  if(dialogLayer||popoverLayer)imports.add('Show');
  ...
  const dateRangePickerFallback = dateRangePicker ? `<div ref={dateRangeRoot} tabindex="-1" class={rangeClasses()}><button type="button" data-navigation="previous">Previous</button><button type="button" data-navigation="next">Next</button><For each={rangeDays} children={day => <button type="button" data-day={day.iso} onClick={() => selectRangeDay(day.iso)}>{day.label}</button>} /><button type="button" data-reset onClick={resetDateRange}>Reset</button></div>` : null;
  ...
  const fallback = popoverFallback ?? sidebarFallback ?? dateRangePickerFallback ?? datePickerFallback ?? toastFallback ?? commandPaletteFallback ?? autocompleteFallback ?? comboboxFallback ?? sensitiveInputFallback ?? inputGroupFallback ?? dialogFallback ?? menubarFallback ?? tabsFallback ?? radioFallback ?? paginationFallback ?? clipboardFallback ?? (nativeButton
...
const compoundFixtureText = (value: unknown, exported: string): string => {
  ...
  const popoverKeyDown: JSX.EventHandlerUnion<HTMLDivElement, KeyboardEvent> = event => { if (event.key !== ${JSON.stringify(popoverLayer.dismiss.key)}) return; event.preventDefault(); setPopoverOpen(false); };
...
` : ''}${dateRangePicker ? `  type DateRangeDay = {iso: string; label: number};
  const rangeDays: DateRangeDay[] = Array.from({length:${dateRangePicker.calendarButtons}}, (_, index) => { const date = new Date(Date.UTC(2000, 0, index + 1)); return {iso:date.toISOString().slice(0, 10), label:date.getUTCDate()}; });
...
export function emitSolidLibrary({libraryPath, outputPath} = {}) {
  ...
  for (const model of library.models) {
    ...
    const js = source(model, toggle, nativeInput, fieldControl, modelClipboardCopy, modelPaginationControls, modelRadioGroup, modelTabsNavigation, modelMenubarNavigation, modelDialogLayer, modelInputGroupComposition, modelSensitiveInput, modelComboboxCollection, modelAutocompleteCollection, modelCommandPalette, modelToastLifecycle, modelDatePicker, modelDateRangePicker, modelResponsiveSidebar, modelPopoverLayer), dts = declaration(model, toggle, nativeInput, modelClipboardCopy, modelPaginationControls, modelMenubarNavigation, modelDialogLayer, modelInputGroupComposition, modelSensitiveInput, modelComboboxCollection, modelAutocompleteCollection, modelCommandPalette, modelToastLifecycle, modelDatePicker, modelDateRangePicker, modelPopoverLayer), variants = model.draftImplementation.semanticVariants ?? [];
