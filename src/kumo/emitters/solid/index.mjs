...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy) {
  ...
  const semantic = (nativeInputElement || clipboardCopy) ? '' : [...variants].sort((a, b) => b.when.length - a.when.length).map(variant => `  if (${variant.when.map(predicate).join(' && ') || 'true'}) return (${node(variant.tree)});`).join('\n');
