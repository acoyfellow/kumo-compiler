...
const KUMO_SELECT_TRIGGER_CLASS='group flex w-max shrink-0 items-center select-none border-0 shadow-xs focus:outline-none focus-visible:ring-2 focus-visible:ring-kumo-brand cursor-pointer disabled:cursor-not-allowed disabled:text-kumo-subtle h-9 gap-1.5 rounded-lg px-3 text-base bg-kumo-base !text-kumo-default ring not-disabled:hover:bg-kumo-tint disabled:bg-kumo-base/50 disabled:!text-kumo-default/70 ring-kumo-line data-[state=open]:bg-kumo-base justify-between font-normal focus:opacity-100 focus:ring-kumo-focus/50 focus-visible:ring-inset *:in-focus:opacity-100';
...
const KUMO_COMMAND_INPUT_GROUP_CLASS='flex items-center gap-3 bg-kumo-base px-4 py-3 focus-within:ring-2 focus-within:ring-kumo-brand';
...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls, radioGroup, tabsNavigation, menubarNavigation, dialogLayer, inputGroupComposition, sensitiveInput, comboboxCollection, autocompleteCollection, commandPalette, toastLifecycle, datePicker, dateRangePicker, responsiveSidebar, popoverLayer, dropdownMenuLayer, selectCollection) {
  ...
  const checkboxHiddenInput = isCheckbox ? `<input style=${JSON.stringify(KUMO_CHECKBOX_HIDDEN_INPUT_STYLE)} tabindex="-1" type="checkbox" aria-hidden="true" checked={checked()} disabled={Boolean(props.${toggle?.disabled.prop}) || undefined} />` : '';
  const checkboxBox = withMt => `<span data-kumo-component="Checkbox" class={mergeStyles(${JSON.stringify(KUMO_CHECKBOX_BOX_CLASS)}${withMt ? ' + (props.label !== undefined ? " mt-0.5" : "")' : ''}${toggleVariantArgs})}${toggleData} aria-label={props["aria-label"] as string} role="checkbox" aria-checked={currentIndeterminate() ? "mixed" : checked()} aria-disabled={Boolean(props.${toggle?.disabled.prop}) || undefined} tabIndex={props.${toggle?.disabled.prop} ? undefined : 0} onClick={toggleChecked} onKeyDown={toggleOnKeyDown}>${checkboxIndicator}</span>`;
  const checkboxFallback = isCheckbox ? `props.label !== undefined ? (<div class=${JSON.stringify(KUMO_CHECKBOX_LABEL_WRAPPER_CLASS)}><label class={mergeStyles(${JSON.stringify(KUMO_CHECKBOX_LABEL_CLASS)}, props.${toggle?.disabled.prop} ? "cursor-not-allowed" : "cursor-pointer")}>${checkboxBox(true)}${checkboxHiddenInput}<span class=${JSON.stringify(KUMO_CHECKBOX_LABEL_TEXT_CLASS)}>{props.label as string}</span></label></div>) : (<>${checkboxBox(false)}${checkboxHiddenInput}</>)` : null;
  const nativeInputClass = nativeInputElement === 'textarea' ? KUMO_INPUTAREA_CLASS : KUMO_INPUT_CLASS;
  const nativeInputBare = nativeInputElement ? nativeInputElement === 'textarea'
    ? `<textarea {...native} class={mergeStyles(${JSON.stringify(nativeInputClass)}, styles.root, props.class)} disabled={Boolean(props.disabled)} onInput={nativeInputHandler}>{props.${nativeInput?.uncontrolled.prop} as string | undefined}</textarea>`
    : `<input {...native} class={mergeStyles(${JSON.stringify(nativeInputClass)}, styles.root, props.class)} value={props.${nativeInput?.uncontrolled.prop} as string | number | string[] | undefined} disabled={Boolean(props.disabled)} onInput={nativeInputHandler}></input>` : null;
  ...
  const nativeInputFallback = nativeInputBare && ownedControlId ? `props.label != null ? <div><label for=${JSON.stringify(ownedControlId)}>{props.label as JSX.Element}</label>${nativeInputElement === 'textarea' ? `<textarea {...native} id=${JSON.stringify(ownedControlId)} class={mergeStyles(${JSON.stringify(nativeInputClass)}, styles.root, props.class)} disabled={Boolean(props.disabled)} onInput={nativeInputHandler}>{props.${nativeInput?.uncontrolled.prop} as string | undefined}</textarea>` : `<input {...native} id=${JSON.stringify(ownedControlId)} class={mergeStyles(${JSON.stringify(nativeInputClass)}, styles.root, props.class)} value={props.${nativeInput?.uncontrolled.prop} as string | number | string[] | undefined} disabled={Boolean(props.disabled)} onInput={nativeInputHandler}></input>`}</div> : ${nativeInputBare}` : nativeInputBare;
  ...
  const dialogFallback = dialogLayer ? `<><button ref={triggerElement} type="button" data-kumo-component="Dialog" data-kumo-part="trigger" aria-haspopup="dialog" class={dialogOpen() ? ${JSON.stringify(KUMO_OVERLAY_BUTTON_CLASS)} : undefined} onClick={() => setDialogOpen(true)}>{props.Trigger != null ? fixtureText(props.Trigger) : compoundFixtureText(props.fixture, ".Trigger")}</button><Show when={dialogOpen()} children={<div data-base-ui-portal=""><div role="presentation" style="position:fixed;inset:0;user-select:none"></div><div role="presentation" class=${JSON.stringify(KUMO_DIALOG_BACKDROP_CLASS)}></div><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span><div ref={dialogElement} role="dialog" tabindex="-1" class=${JSON.stringify(KUMO_DIALOG_CONTENT_CLASS)} style="transition-property:scale,opacity;transition-timing-function:var(--default-transition-timing-function);--tw-shadow:0 20px 25px -5px rgb(0 0 0 / 0.03),0 8px 10px -6px rgb(0 0 0 / 0.03)"><h2>{compoundFixtureText(props.fixture, ".Title")}</h2><p>{compoundFixtureText(props.fixture, ".Description")}</p><button type="button" data-kumo-component="Dialog" data-kumo-part="close" class=${JSON.stringify(KUMO_OVERLAY_BUTTON_CLASS)} onClick={() => setDialogOpen(false)}>{compoundFixtureText(props.fixture, ".Close")}</button></div><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span></div>} /></>` : null;
  ...
  const selectFallback = selectCollection ? `<div class=${JSON.stringify(KUMO_SELECT_ROOT_CLASS)}><button ref={selectTrigger} type="button" tabindex="0" role="combobox" aria-expanded={selectOpen()} aria-haspopup="listbox" aria-label={props["aria-label"] as string} data-kumo-component="Select" data-kumo-part="trigger" class=${JSON.stringify(KUMO_SELECT_TRIGGER_CLASS)} onClick={() => setSelectOpen(!selectOpen())} onKeyDown={selectKeyDown}><span class=${JSON.stringify(KUMO_SELECT_VALUE_CLASS)}>{selectLabel()}</span><span aria-hidden="true" class=${JSON.stringify(KUMO_SELECT_ICON_CLASS)}>${KUMO_SELECT_CHEVRON_SVG}</span></button><input tabindex="-1" aria-hidden="true" style=${JSON.stringify(KUMO_CHECKBOX_HIDDEN_INPUT_STYLE)} value={selectValue() != null ? String(selectValue()) : ""} /><Show when={selectOpen()} children={<div role="listbox"><For each={selectOptions()} children={(item, index) => <div ref={element => { selectElements[index()] = element; }} role="option" tabindex="-1" aria-disabled={item.disabled || undefined} onClick={() => selectOption(item, index())} onKeyDown={selectKeyDown}>{item.label}</div>} /></div>} /></div>` : null;
  const dropdownFallback = dropdownMenuLayer ? `<><Show when={dropdownOpen()} children={<span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span>} /><button ref={dropdownTrigger} type="button" tabindex="0" aria-haspopup="menu" aria-expanded={dropdownOpen()} data-kumo-component={dropdownOpen() ? "Button" : undefined} class={dropdownOpen() ? ${JSON.stringify(KUMO_OVERLAY_BUTTON_CLASS)} : undefined} onClick={() => setDropdownOpen(!dropdownOpen())} onKeyDown={dropdownTriggerKeyDown}>{dropdownTriggerText()}</button><Show when={dropdownOpen()} children={<><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span><div data-base-ui-portal=""><div role="presentation" style="position:absolute;left:0;top:0;transform:translate(-19px,44px)"><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span><div role="menu" class=${JSON.stringify(KUMO_DROPDOWN_CONTENT_CLASS)}><For each={dropdownItems()} children={item => <div role="menuitem" tabindex="-1" data-kumo-component="DropdownMenu" data-kumo-part="item" data-disabled={item.disabled ? "" : undefined} class=${JSON.stringify(KUMO_DROPDOWN_ITEM_CLASS)} onPointerDown={event => selectDropdownItem(item, event)} onKeyDown={event => dropdownItemKeyDown(event, item)}>{item.text}</div>} /></div><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span></div></div><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span><span style="clip-path:inset(50%);position:fixed;top:0;left:0"></span><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span></>} /></>` : null;
  // Combobox/Autocomplete SSR render exactly what React canonical does: the visually-hidden
  // native form <input> (Base UI portals the visible trigger + listbox at runtime, so they
  // are absent from static markup). Matching that hidden input gives A+B+C parity; the
  // gated listbox stays inert at SSR (fixture-driven) and only materializes client-side.
  const comboboxFallback = comboboxCollection ? `<><input ref={comboboxInput} aria-hidden="true" tabindex="-1" style=${JSON.stringify(KUMO_CHECKBOX_HIDDEN_INPUT_STYLE)} value={comboboxValue()} onClick={() => setComboboxOpen(true)} onKeyDown={comboboxKeyDown} /><Show when={comboboxOpen() && comboboxItems().length > 0} children={<ul role="listbox"><For each={comboboxItems()} children={(item, index) => <li role="option" data-value={item.value} aria-selected={highlightedIndex() === index()}>{fixtureText(item)}</li>} /></ul>} /></>` : null;
  ...
  const sensitiveInputFallback = sensitiveInput ? `<div><div role="button" data-kumo-component="SensitiveInput" data-kumo-part="masked-container" tabindex="0" class=${JSON.stringify(KUMO_SENSITIVE_CONTAINER_CLASS)} aria-label="Sensitive value, masked." aria-disabled="false" onClick={revealSensitive}><input ref={sensitiveInputElement} type="password" readonly tabindex="-1" aria-hidden="true" aria-label={props.label as string} class=${JSON.stringify(KUMO_SENSITIVE_INPUT_CLASS)} value={sensitiveValue()} onInput={sensitiveOnInput} onKeyDown={sensitiveOnKeyDown} /><span class=${JSON.stringify(KUMO_SENSITIVE_MASK_CLASS)} aria-hidden="true"><span class="relative"><span class=${JSON.stringify(KUMO_SENSITIVE_DOTS_CLASS)}>\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022</span><span class=${JSON.stringify(KUMO_SENSITIVE_REVEAL_CLASS)}>Click to reveal</span></span></span><button type="button" data-kumo-component="SensitiveInput" data-kumo-part="toggle-visibility" aria-label="Reveal value" tabindex="-1" class=${JSON.stringify(KUMO_SENSITIVE_EYE_CLASS)} onClick={revealSensitive}>${KUMO_SENSITIVE_EYE_SVG}</button><button type="button" data-kumo-component="SensitiveInput" data-kumo-part="copy" aria-label="Copy to clipboard" class=${JSON.stringify(KUMO_SENSITIVE_COPY_CLASS)} onClick={copySensitive}>Copy</button></div><span class="sr-only">Click or press Enter to reveal.</span><span class="sr-only" aria-live="polite">{sensitiveAnnouncement()}</span></div>` : null;
  ...
  const toastFallback = toastLifecycle ? `props.children != null ? <div data-kumo-component="Toasty">{props.children}<button type="button" data-notify aria-label="Notify" onClick={notifyToast}></button>{toastVisible() ? <div role="status" aria-live="polite"><strong>Saved</strong><span>Changes saved</span><button type="button" data-toast-action onClick={toastAction}>Action</button><button type="button" aria-label="Close" onClick={closeToast}>Close</button></div> : undefined}</div> : <button type="button" data-kumo-component="Button" class=${JSON.stringify(KUMO_OVERLAY_BUTTON_CLASS)} onClick={notifyToast}>Notify</button>` : null;
  const commandPaletteFallback = commandPalette ? `props.text !== undefined ? <span><For each={highlightSegments()} children={segment => segment.mark ? <mark>{segment.text}</mark> : segment.text} /></span> : <Show when={paletteOpen()} fallback={<></>} children={<div data-base-ui-portal=""><div role="presentation" style="position:fixed;inset:0;user-select:none"></div><div role="presentation" class=${JSON.stringify(KUMO_COMMAND_BACKDROP_CLASS)}></div><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span><div role="dialog" tabindex="-1" class=${JSON.stringify(KUMO_COMMAND_DIALOG_CLASS)} style="transition-property:scale,opacity;transition-timing-function:var(--default-transition-timing-function)"><div class=${JSON.stringify(KUMO_COMMAND_PANEL_CLASS)}><div class=${JSON.stringify(KUMO_COMMAND_INPUT_GROUP_CLASS)}>${KUMO_COMMAND_SEARCH_SVG}<span role="button" aria-label="Dismiss" style="clip-path:inset(50%);overflow:hidden;white-space:nowrap;border:0;padding:0;width:1px;height:1px;margin:-1px;position:absolute"></span><input ref={commandPaletteInput} autofocus role="combobox" aria-expanded="true" aria-haspopup="listbox" aria-autocomplete="list" autocomplete="off" spellcheck="false" placeholder={palettePlaceholder()} class=${JSON.stringify(KUMO_COMMAND_INPUT_CLASS)} value={paletteValue()} onInput={commandPaletteOnInput} onKeyDown={commandPaletteKeyDown} /></div><div class=${JSON.stringify(KUMO_COMMAND_LIST_CLASS)}><For each={paletteItems()} children={(item, index) => <div role="option" class=${JSON.stringify(KUMO_COMMAND_ITEM_CLASS)} data-highlighted={highlightedPaletteIndex() === index() ? "" : undefined}>{item.label}</div>} /></div><input tabindex="-1" aria-hidden="true" value={paletteValue()} style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)} /></div></div><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span></div>} />` : null;
   const meterFallback = model.component === 'meter' ? `<div class=${JSON.stringify(KUMO_METER_ROOT_CLASS)} role="meter" aria-valuenow={props.value as number} aria-valuemin={(props.min as number) ?? 0} aria-valuemax={(props.max as number) ?? 100} aria-valuetext={Math.round((((props.value as number) ?? 0) - ((props.min as number) ?? 0)) / (((((props.max as number) ?? 100) - ((props.min as number) ?? 0)) as number) || 1) * 100) + "%"}><div class=${JSON.stringify(KUMO_METER_HEADER_CLASS)}><span role="presentation" class=${JSON.stringify(KUMO_METER_LABEL_CLASS)}>{props.label as JSX.Element}</span>{(props.showValue as boolean) !== false ? <span aria-hidden="true" class=${JSON.stringify(KUMO_METER_VALUE_CLASS)}>{(props.customValue as JSX.Element) ?? ((props.value as number) + "%")}</span> : null}</div><div class=${JSON.stringify(KUMO_METER_TRACK_CLASS)}><div class=${JSON.stringify(KUMO_METER_FILL_CLASS)} style={{ "inset-inline-start": "0", height: "inherit", width: (props.value as number) + "%" }} /></div><span role="presentation" style="clip-path:inset(50%);overflow:hidden;white-space:nowrap;border:0;padding:0;width:1px;height:1px;margin:-1px;position:fixed;top:0;left:0">x</span></div>` : null;
  const fallback = visualSimpleFallback ?? tableOfContentsFallback ?? selectFallback ?? dropdownFallback ?? popoverFallback ?? sidebarFallback ?? dateRangePickerFallback ?? datePickerFallback ?? toastFallback ?? commandPaletteFallback ?? autocompleteFallback ?? comboboxFallback ?? sensitiveInputFallback ?? inputGroupFallback ?? dialogFallback ?? menubarFallback ?? tabsFallback ?? radioFallback ?? paginationFallback ?? clipboardFallback ?? meterFallback ?? (nativeButton
    ? `<button id={props.id as string} class={mergeStyles(${nativeButtonVariantExpression(nativeButton, 'props.variant')}, props.class)} style={${nativeButtonEmphasisStyleExpression(nativeButton.emphasis, 'props.variant')}} name={props.name as string} value={props.value as string} data-probe={props["data-probe"] as string} aria-label={props["aria-label"] as string} type={(props.type as JSX.ButtonHTMLAttributes<HTMLButtonElement>["type"]) ?? "button"} disabled={Boolean(props.disabled || props.loading)} onClick={props.onClick as JSX.EventHandlerUnion<HTMLButtonElement, MouseEvent>}>{(${nativeButtonEmphasisCondition(nativeButton.emphasis, 'props.variant')}) ? <><span aria-hidden="true" class=${JSON.stringify(nativeButton.emphasis.overlayClass)} /><span class=${JSON.stringify(nativeButton.emphasis.wrapperClass)}>{props.loading ? <svg aria-hidden="true" /> : undefined}{props.children}</span></> : <>{props.loading ? <svg aria-hidden="true" /> : undefined}{props.children}</>}</button>`
    : nativeInputFallback ?? providedFieldFallback ?? checkboxFallback ?? toggleFallback ?? node(root));
