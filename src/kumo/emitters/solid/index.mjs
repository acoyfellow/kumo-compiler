...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls, radioGroup, tabsNavigation, menubarNavigation, dialogLayer, inputGroupComposition, sensitiveInput, comboboxCollection, autocompleteCollection, commandPalette, toastLifecycle, datePicker, dateRangePicker, responsiveSidebar, popoverLayer, dropdownMenuLayer, selectCollection) {
  ...
  const checkboxIndicator = isCheckbox ? `<span class=${JSON.stringify(KUMO_CHECKBOX_INDICATOR_CLASS)} data-checked={checked() ? "" : undefined} data-unchecked={checked() ? undefined : ""} data-indeterminate={currentIndeterminate() ? "" : undefined} innerHTML={currentIndeterminate() ? ${JSON.stringify(KUMO_CHECKBOX_MINUS_SVG)} : ${JSON.stringify(KUMO_CHECKBOX_CHECK_SVG)}} />` : '';
  ...
  const clipboardFallback = clipboardCopy ? `<div class=${JSON.stringify(KUMO_CLIPBOARD_ROOT_CLASS)}><span class=${JSON.stringify(KUMO_CLIPBOARD_TEXT_CLASS)}>{props.${clipboardCopy.copySource.fallback} as JSX.Element}</span><button data-kumo-component="Button" type="button" class=${JSON.stringify(KUMO_CLIPBOARD_BUTTON_CLASS)} aria-label="Copy to clipboard" onClick={copyText}><span class="contents"><span class=${JSON.stringify(KUMO_CLIPBOARD_CHECK_SPAN_CLASS)} innerHTML={${JSON.stringify(KUMO_CLIPBOARD_CHECK_SVG)}} /><span class=${JSON.stringify(KUMO_CLIPBOARD_COPY_SPAN_CLASS)} innerHTML={${JSON.stringify(KUMO_CLIPBOARD_COPY_SVG)}} /></span></button><span class="sr-only" aria-live="polite">{copyStatus()}</span></div>` : null;
