...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls, radioGroup, tabsNavigation, menubarNavigation, dialogLayer, inputGroupComposition, sensitiveInput, comboboxCollection, autocompleteCollection, commandPalette, toastLifecycle, datePicker, dateRangePicker, responsiveSidebar, popoverLayer, dropdownMenuLayer, selectCollection) {
  ...
  const fallback = selectFallback ?? dropdownFallback ?? popoverFallback ?? sidebarFallback ?? dateRangePickerFallback ?? datePickerFallback ?? toastFallback ?? commandPaletteFallback ?? autocompleteFallback ?? comboboxFallback ?? sensitiveInputFallback ?? inputGroupFallback ?? dialogFallback ?? menubarFallback ?? tabsFallback ?? radioFallback ?? paginationFallback ?? clipboardFallback ?? (nativeButton
...
const compoundFixtureText = (value: unknown, exported: string): string => {
  ...
  const selectOption = (item: SelectOption) => { if (props.disabled || item.disabled) return; let next: unknown = item.value; if (props.multiple) { const current=Array.isArray(selectValue()) ? selectValue() as unknown[] : []; next=current.some(value=>semanticEqual(value,item.value)) ? current : [...current,item.value]; } if (!hasValue) setOwnValue(next); (props.onSelect as ((value:unknown)=>void)|undefined)?.(item.value); (props.onValueChange as ((value:unknown)=>void)|undefined)?.(next); if (!props.multiple) { setSelectOpen(false); queueMicrotask(()=>selectTrigger?.focus()); } };
  const selectKeyDown = (event: KeyboardEvent) => { if (props.disabled) return; const items=selectOptions(); if (event.key === "Tab") { if (selectOpen()) setSelectOpen(false); return; } if (event.key === "Escape") { event.preventDefault(); setSelectOpen(false); queueMicrotask(()=>selectTrigger?.focus()); return; } if (!selectOpen() && event.key === "ArrowDown") { event.preventDefault(); setSelectOpen(true); highlightSelect(Math.max(0, items.findIndex(item=>semanticEqual(item.value,selectValue())))); return; } if (!selectOpen()) return; let index=-1; if (event.key === "Home") index=0; else if (event.key === "End") index=items.length-1; else if (event.key === "ArrowDown") index=activeSelectIndex()+1; else if (event.key.length === 1) index=items.findIndex(item=>!item.disabled && item.label.toLowerCase().startsWith(event.key.toLowerCase())); else return; if(index>=0){event.preventDefault();highlightSelect(index);} };
