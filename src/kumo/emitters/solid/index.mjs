...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls, radioGroup, tabsNavigation, menubarNavigation, dialogLayer, inputGroupComposition, sensitiveInput, comboboxCollection, autocompleteCollection, commandPalette, toastLifecycle, datePicker, dateRangePicker, responsiveSidebar, popoverLayer, dropdownMenuLayer, selectCollection) {
  ...
  const toggleControl=toggle?(toggleTag==='span'?`<span${toggleClass}${toggleData} aria-label={props["aria-label"] as string} role=${JSON.stringify(toggleRole)} aria-checked={currentIndeterminate() ? "mixed" : checked()} aria-disabled={Boolean(props.${toggle.disabled.prop}) || undefined} tabIndex={props.${toggle.disabled.prop} ? undefined : 0} onClick={toggleChecked} onKeyDown={toggleOnKeyDown} />`:`<button${toggleClass}${toggleData} aria-label={props["aria-label"] as string} type="button" role=${JSON.stringify(toggleRole)} aria-checked={checked()} disabled={Boolean(props.${toggle.disabled.prop})} onClick={toggleChecked}><span aria-hidden="true" class=${JSON.stringify(KUMO_SWITCH_THUMB_CLASS)} /></button>`):null;
  ...
  const tabsFallback = tabsNavigation ? `<div role="tablist" class=${JSON.stringify(KUMO_TABS_LIST_CLASS)}><For each={props.tabs as TabItem[]} children={(item, index) => <button ref={element => { tabElements[index()] = element; }} type="button" role="tab" class=${JSON.stringify(KUMO_TABS_TRIGGER_CLASS)} aria-selected={selectedValue() === item.value} tabindex={focusedIndex() === index() ? 0 : -1} onClick={() => commitTab(item.value)} onFocus={() => setFocusedIndex(index())} onKeyDown={event => tabKeyDown(event, index())}>{selectedValue() === item.value ? <span aria-hidden="true" class=${JSON.stringify(KUMO_TABS_INDICATOR_CLASS)} /> : null}{item.label}</button>} /></div>` : null;
