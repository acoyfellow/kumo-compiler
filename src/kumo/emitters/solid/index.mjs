...
function declaration(model, toggle, nativeInput, clipboardCopy, paginationControls, menubarNavigation, dialogLayer, inputGroupComposition, sensitiveInput) {
  const nativeButton=Boolean(model.interactions?.nativeButton);
  const nativeInputElement=nativeInput?.requirements.dom?.[0];
  const modelProps = sensitiveInput ? [
  ...
  const props = modelProps.map(item => `  ${JSON.stringify(item.name)}${item.required ? '' : '?'}: ${item.type === 'number' ? 'number' : item.type === 'dialog-open' ? 'boolean' : item.type === 'dialog-change' ? '(open: boolean) => void' : item.type === 'menubar-options' ? 'Array<{ id: string; tooltip: string; icon: JSX.Element; onClick?: () => void }>' : item.type === 'menubar-active' ? 'number | string' : item.type === 'pagination-mode' ? '"simple" | "dropdown"' : item.type === 'pagination-labels' ? '{ navigation?: string; previousPage?: string; nextPage?: string }' : item.type === 'pagination-callback' ? '(page: number) => void' : item.type === 'sensitive-value-change' ? '(value: string) => void' : nativeButton&&['disabled','loading'].includes(item.name)||toggle&&[toggle.controlledProp,toggle.defaultProp,toggle.disabled.prop,toggle.indeterminate?.prop].includes(item.name)?'boolean':safeType(item.type).includes('callback') ? toggle&&item.name==='onCheckedChange'?'(checked: boolean) => void':clipboardCopy&&item.name==='onCopy'?'() => void':'(...args: unknown[]) => void' : clipboardCopy&&[clipboardCopy.copySource.prop,clipboardCopy.copySource.fallback].includes(item.name)?'string':'unknown'};`).join('\n');
...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls, radioGroup, tabsNavigation, menubarNavigation, dialogLayer, inputGroupComposition, sensitiveInput) {
  ...
  const nativeInputElement=nativeInput?.requirements.dom?.[0];
  if(nativeButton||paginationControls||radioGroup||tabsNavigation||menubarNavigation||dialogLayer||inputGroupComposition||sensitiveInput)imports.add('mergeProps');
  if(toggle || clipboardCopy || paginationControls || radioGroup || tabsNavigation || dialogLayer || inputGroupComposition || sensitiveInput)imports.add('createSignal');
  ...
  const fallback = sensitiveInputFallback ?? inputGroupFallback ?? dialogFallback ?? menubarFallback ?? tabsFallback ?? radioFallback ?? paginationFallback ?? clipboardFallback ?? (nativeButton
  ...
  const semantic = (nativeInputElement || clipboardCopy || paginationControls || radioGroup || tabsNavigation || menubarNavigation || dialogLayer || inputGroupComposition || sensitiveInput) ? '' : [...variants].sort((a, b) => b.when.length - a.when.length).map(variant => `  if (${variant.when.map(predicate).join(' && ') || 'true'}) return (${node(variant.tree)});`).join('\n');
...
export function emitSolidLibrary({libraryPath, outputPath} = {}) {
  ...
  for (const model of library.models) {
    ...
    const js = source(model, toggle, nativeInput, fieldControl, modelClipboardCopy, modelPaginationControls, modelRadioGroup, modelTabsNavigation, modelMenubarNavigation, modelDialogLayer, modelInputGroupComposition, modelSensitiveInput), dts = declaration(model, toggle, nativeInput, modelClipboardCopy, modelPaginationControls, modelMenubarNavigation, modelDialogLayer, modelInputGroupComposition, modelSensitiveInput), variants = model.draftImplementation.semanticVariants ?? [];
