...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls, radioGroup) {
  ...
  const nativeInputElement=nativeInput?.requirements.dom?.[0];
  if(nativeButton||paginationControls||radioGroup)imports.add('mergeProps');
  if(toggle || clipboardCopy || paginationControls || radioGroup)imports.add('createSignal');
  if(radioGroup)imports.add('For');
...
export function emitSolidLibrary({libraryPath, outputPath} = {}) {
  ...
  for (const model of library.models) {
    ...
    const js = source(model, toggle, nativeInput, fieldControl, modelClipboardCopy, modelPaginationControls, modelRadioGroup), dts = declaration(model, toggle, nativeInput, modelClipboardCopy, modelPaginationControls), variants = model.draftImplementation.semanticVariants ?? [];
