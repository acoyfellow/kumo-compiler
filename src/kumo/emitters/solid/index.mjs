...
function source(model, toggle, nativeInput) {
  ...
  const nativeInputFallback = nativeInputElement ? `<${nativeInputElement} {...native} class={mergeStyles(styles.root, props.class)} value={props.${nativeInput?.uncontrolled.prop} as string | number | string[] | undefined} disabled={Boolean(props.disabled)} onInput={nativeInputHandler}></${nativeInputElement}>` : null;
  const fallback = nativeButton
    ...
    : nativeInputFallback ?? toggleFallback ?? node(root);
...
export function emitSolidLibrary({libraryPath, outputPath} = {}) {
  ...
  for (const model of library.models) {
    const toggle = toggleFor(model), nativeInput = nativeInputs.get(model.component) ?? null;
    const js = source(model, toggle, nativeInput), dts = declaration(model, toggle, nativeInput), variants = model.draftImplementation.semanticVariants ?? [];
