...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls, radioGroup, tabsNavigation, menubarNavigation, dialogLayer, inputGroupComposition, sensitiveInput, comboboxCollection, autocompleteCollection, commandPalette, toastLifecycle, datePicker, dateRangePicker, responsiveSidebar, popoverLayer, dropdownMenuLayer, selectCollection) {
  ...
  const gridFallback = grid ? `<div class={mergeStyles("grid", (${JSON.stringify(KUMO_GRID_VARIANT_CLASSES)} as Record<string,string>)[props.variant as string], (${JSON.stringify(KUMO_GRID_GAP_CLASSES)} as Record<string,string>)[(props.gap as string) ?? "base"] ?? "${KUMO_GRID_GAP_CLASSES.base}", props.className as string | undefined, props.class as string | undefined)}>{props.children}</div>` : null;
  const layerCard = model.component === 'layer-card';
  // LayerCard root: detect Primary/Secondary sub-parts (data-kumo-layer-part)
  // after mount and swap between plain and layered literal class strings (both
  // already in the prebuilt CSS).
  const layerCardFallback = layerCard ? `<div ref={el => { layerCardRoot = el; queueMicrotask(() => setLayerCardMode(el.querySelector('[data-kumo-layer-part]') ? "layered" : "plain")); }} class={mergeStyles(layerCardMode() === "layered" ? "flex w-full flex-col overflow-hidden rounded-lg bg-kumo-elevated text-base ring ring-kumo-hairline" : "overflow-hidden rounded-lg bg-kumo-base shadow-xs ring ring-kumo-line", props.className as string | undefined, props.class as string | undefined)}>{props.children}</div>` : null;
  ...
  if(layerCard)imports.add('createSignal');
  ...
  const tooltipFallback=tooltip?`<>{props.asChild ? <span style="display:contents" ref={el => { tooltipAnchorEl = el; }}><KumoMergeTriggerContext.Provider value={{"data-base-ui-tooltip-trigger": ""}}>{props.children}</KumoMergeTriggerContext.Provider></span> : <span style="display:contents" ref={el => { tooltipAnchorEl = el; }}><button class="inline-flex items-center bg-transparent border-none shadow-none p-0 m-0 h-auto min-h-0 leading-[0] cursor-default" type="button" data-base-ui-tooltip-trigger="">{props.children}</button></span>}<Show when={props.open !== undefined ? Boolean(props.open) : Boolean(props.defaultOpen)} children={typeof document !== "undefined" ? <Portal><div data-base-ui-portal="" data-base-ui-inert=""><div ref={el => { const positioner = el; queueMicrotask(() => { const host = tooltipAnchorEl; if (!host) return; const trigger = host.querySelector ? host.querySelector("[data-base-ui-tooltip-trigger]") : null; const anchorEl = trigger || host.firstElementChild || host; const popup = positioner.querySelector("[data-base-ui-focusable]"); if (!anchorEl || !popup) return; const t = anchorEl.getBoundingClientRect(); const pop = popup.getBoundingClientRect(); const left = Math.round((t.left + t.width / 2 - pop.width / 2) * 100) / 100; const top = Math.round((t.top - pop.height - 10) * 100) / 100; positioner.style.transform = "translate(" + left + "px," + top + "px)"; positioner.style.setProperty("--available-width", (document.documentElement.clientWidth - 10) + "px"); positioner.style.setProperty("--available-height", (document.documentElement.clientHeight - 11) + "px"); }); }} data-open="" data-side="top" data-align="center" role="presentation" class=${JSON.stringify(KUMO_TOOLTIP_POSITIONER_CLASS)} style="position:absolute;left:0;top:0;transform:translate(0px,-10px)"><div data-open="" data-side="top" data-align="center" tabindex="-1" data-base-ui-focusable="" class=${JSON.stringify(KUMO_TOOLTIP_POPUP_CLASS)}><div data-open="" data-side="top" data-align="center" aria-hidden="true" class=${JSON.stringify(KUMO_TOOLTIP_ARROW_CLASS)} style="position:absolute;left:12px">${KUMO_TOOLTIP_ARROW_SVG}</div>{props.content as JSX.Element}</div></div></div></Portal> : undefined} /></>`:null;
  ...
  const checkboxHiddenInput = withId => isCheckbox ? `<input${withId ? ` id=${JSON.stringify(checkboxInputId)}` : ''} style=${JSON.stringify(KUMO_CHECKBOX_HIDDEN_INPUT_STYLE)} tabindex="-1" type="checkbox" aria-hidden="true" attr:checked={checked() ? "" : undefined} disabled={Boolean(props.${toggle?.disabled.prop}) || undefined} />` : '';
  ...
  const paginationFallback = paginationControls ? `<div data-slot="pagination" class=${JSON.stringify(KUMO_PAGINATION_ROOT_CLASS)}><div aria-live="polite" aria-atomic="true" data-slot="pagination-info" class=${JSON.stringify(KUMO_PAGINATION_INFO_CLASS)}>Showing <span class="tabular-nums">{String(((props.page as number) - 1) * (props.perPage as number) + 1) + "-" + String(Math.min((props.page as number) * (props.perPage as number), props.totalCount as number))}</span> of <span class="tabular-nums">{props.totalCount as number}</span></div><div data-slot="pagination-controls" class=${JSON.stringify(KUMO_PAGINATION_CONTROLS_CLASS)}><nav ref={navEl} aria-label={(props.labels as {navigation?: string} | undefined)?.navigation ?? "Pagination"}><div data-slot="input-group" data-focus-mode="individual" class=${JSON.stringify(KUMO_PAGINATION_GROUP_CLASS)}>{props.fixtureMode !== "simple" ? <button data-kumo-component="Button" type="button" class={${JSON.stringify(KUMO_PAGINATION_BUTTON_CLASS)} + (clampPage(props.page as number) === 1 ? " cursor-not-allowed opacity-50" : "")} aria-label="First page" disabled={clampPage(props.page as number) === 1} onClick={() => proposePage(1)}><span class="contents">${KUMO_PAGINATION_FIRST_SVG}</span></button> : undefined}<button data-kumo-component="Button" type="button" class={${JSON.stringify(KUMO_PAGINATION_BUTTON_CLASS)} + (clampPage(props.page as number) === 1 ? " cursor-not-allowed opacity-50" : "")} aria-label={(props.labels as {previousPage?: string} | undefined)?.previousPage ?? "Previous page"} disabled={clampPage(props.page as number) === 1} onClick={() => proposePage(currentPage() - 1)}><span class="contents">${KUMO_PAGINATION_PREV_SVG}</span></button>{props.fixtureMode !== "simple" ? <input aria-label="Page number" class=${JSON.stringify(KUMO_PAGINATION_INPUT_CLASS)} style="width:50px" attr:value={inputValue()} onInput={pageInput} onKeyDown={pageKeyDown} onBlur={pageBlur} /> : undefined}<button data-kumo-component="Button" type="button" class={${JSON.stringify(KUMO_PAGINATION_BUTTON_CLASS)} + (clampPage(props.page as number) === maxPage() ? " cursor-not-allowed opacity-50" : "")} aria-label={(props.labels as {nextPage?: string} | undefined)?.nextPage ?? "Next page"} disabled={clampPage(props.page as number) === maxPage()} onClick={() => proposePage(currentPage() + 1)}><span class="contents">${KUMO_PAGINATION_NEXT_SVG}</span></button>{props.fixtureMode !== "simple" ? <button data-kumo-component="Button" type="button" class={${JSON.stringify(KUMO_PAGINATION_BUTTON_CLASS)} + (clampPage(props.page as number) === maxPage() ? " cursor-not-allowed opacity-50" : "")} aria-label="Last page" disabled={clampPage(props.page as number) === maxPage()} onClick={() => proposePage(maxPage())}><span class="contents">${KUMO_PAGINATION_LAST_SVG}</span></button> : undefined}</div></nav></div></div>` : null;
  ...
  const toastFallback = toastLifecycle ? `<ToastManagerContext.Provider value={manager}>{props.children}<Show when={toastMounted()} children={<Portal><div tabindex="-1" role="region" aria-live="polite" aria-atomic="false" aria-relevant="additions text" aria-label="Notifications" data-kumo-component="Toasty" class="fixed top-auto right-4 bottom-4 z-1 mx-auto flex w-[calc(100%-2rem)] sm:right-8 sm:bottom-8 sm:w-[340px]"><For each={toasts()} children={toast => <Toast toast={toast} defaultVariant={props.variant as KumoToastVariant} onClose={id => manager.close(id)} onAction={props.onAction as (() => void) | undefined} />} /></div></Portal>} />` + `</ToastManagerContext.Provider>` : null;
  ...
  const fallback = tooltipFallback ?? labelFallback ?? surfaceFallback ?? gridFallback ?? layerCardFallback ?? visualSimpleFallback ?? bannerFallback ?? collapsibleFallback ?? tableOfContentsFallback ?? selectFallback ?? dropdownFallback ?? popoverFallback ?? sidebarFallback ?? dateRangePickerFallback ?? datePickerFallback ?? toastFallback ?? commandPaletteFallback ?? autocompleteFallback ?? comboboxFallback ?? sensitiveInputFallback ?? inputGroupFallback ?? dialogFallback ?? menubarFallback ?? tabsFallback ?? radioFallback ?? paginationFallback ?? clipboardFallback ?? meterFallback ?? (nativeButton
  ...
  const parts = compoundParts(model);
  const partSources = parts.map(part => { const override = compoundPartOverride(model.component, part.path); const tag = override?.tag ?? 'div'; const attributes = override ? [override.className ? `class=${JSON.stringify(override.className)}` : '', override.dataAttr ? `${override.dataAttr}=""` : ''].filter(Boolean).join(' ') : `data-kumo-part=${JSON.stringify(part.path)}`; return `export function ${part.symbol}(props: CompoundPartProps): JSX.Element {\n  const [local, native] = splitProps(props, ["children"]);\n  return <${tag} {...native} ${attributes}>{local.children}</${tag}>;\n}`; }).join('\n\n');
