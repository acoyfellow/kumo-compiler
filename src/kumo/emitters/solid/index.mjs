...
function declaration(model, toggle) {
...
function source(model, toggle) {
  ...
  const fallback = nativeButton
    ...
    : toggle
      ? `<${root.tag} type=${JSON.stringify(root.tag === 'input' ? 'checkbox' : 'button')} role=${JSON.stringify(root.tag === 'input' ? 'checkbox' : 'switch')} class={mergeStyles(${root.styles.map(x => expr(x)).join(', ')})} disabled={Boolean(props.${toggle.disabled.prop})} aria-checked={${toggle.indeterminate ? `props.${toggle.indeterminate.prop} ? "mixed" : checked()` : 'checked()'}}${root.tag === 'input' ? ' checked={checked()} onChange={toggleChecked}' : ' onClick={toggleChecked}'} />`
      : node(root);
...
export function emitSolidLibrary({libraryPath, outputPath} = {}) {
  ...
  const toggleFor = model => { const state=controlled.get(model.component), control=native.get(model.component); return toggles.has(model.component) && state?.event === 'checked-change' && control?.events.includes('checked-change') && ['input','button'].includes(model.draftImplementation.componentRoot.tag) ? state : null; };
  const output = outputPath ?? path.resolve(here, '../../../../generated/libraries/solid');
  fs.rmSync(output, {recursive:true, force:true}); fs.mkdirSync(output, {recursive:true});
  const components = [];
  for (const model of library.models) {
    const toggle = toggleFor(model);
    const js = source(model, toggle), dts = declaration(model, toggle), variants = model.draftImplementation.semanticVariants ?? [];
