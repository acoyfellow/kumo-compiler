...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls, radioGroup, tabsNavigation, menubarNavigation, dialogLayer, inputGroupComposition, sensitiveInput, comboboxCollection, autocompleteCollection, commandPalette, toastLifecycle, datePicker, dateRangePicker, responsiveSidebar, popoverLayer, dropdownMenuLayer, selectCollection) {
  ...
  const meterFallback = model.component === 'meter' ? `<div class=${JSON.stringify(KUMO_METER_ROOT_CLASS)} role="meter" aria-valuenow={props.value as number} aria-valuemin={(props.min as number) ?? 0} aria-valuemax={(props.max as number) ?? 100}><div class=${JSON.stringify(KUMO_METER_HEADER_CLASS)}><span class=${JSON.stringify(KUMO_METER_LABEL_CLASS)}>{props.label as JSX.Element}</span>{(props.showValue as boolean) !== false ? <span class=${JSON.stringify(KUMO_METER_VALUE_CLASS)}>{(props.customValue as JSX.Element) ?? ((props.value as number) + "%")}</span> : null}</div><div class=${JSON.stringify(KUMO_METER_TRACK_CLASS)}><div class=${JSON.stringify(KUMO_METER_FILL_CLASS)} style={{ width: (props.value as number) + "%" }} /></div></div>` : null;
