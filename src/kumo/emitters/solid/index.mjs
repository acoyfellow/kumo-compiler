...
function declaration(model, toggle, nativeInput, clipboardCopy, paginationControls, menubarNavigation, dialogLayer) {
...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls, radioGroup, tabsNavigation, menubarNavigation, dialogLayer) {
  ...
  const nativeInputElement=nativeInput?.requirements.dom?.[0];
  if(nativeButton||paginationControls||radioGroup||tabsNavigation||menubarNavigation||dialogLayer)imports.add('mergeProps');
  if(toggle || clipboardCopy || paginationControls || radioGroup || tabsNavigation || dialogLayer)imports.add('createSignal');
  ...
  const fallback = dialogFallback ?? menubarFallback ?? tabsFallback ?? radioFallback ?? paginationFallback ?? clipboardFallback ?? (nativeButton
  ...
  const semantic = (nativeInputElement || clipboardCopy || paginationControls || radioGroup || tabsNavigation || menubarNavigation || dialogLayer) ? '' : [...variants].sort((a, b) => b.when.length - a.when.length).map(variant => `  if (${variant.when.map(predicate).join(' && ') || 'true'}) return (${node(variant.tree)});`).join('\n');
...
export function emitSolidLibrary({libraryPath, outputPath} = {}) {
  ...
  for (const model of library.models) {
    ...
    const js = source(model, toggle, nativeInput, fieldControl, modelClipboardCopy, modelPaginationControls, modelRadioGroup, modelTabsNavigation, modelMenubarNavigation, modelDialogLayer), dts = declaration(model, toggle, nativeInput, modelClipboardCopy, modelPaginationControls, modelMenubarNavigation, modelDialogLayer), variants = model.draftImplementation.semanticVariants ?? [];
