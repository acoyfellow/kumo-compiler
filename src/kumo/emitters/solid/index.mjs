...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy) {
  ...
  if(toggle || clipboardCopy)imports.add('createSignal');
  ...
  const clipboardFallback = clipboardCopy ? `<div>{props.${clipboardCopy.copySource.fallback} as JSX.Element}<button type="button" onClick={copyText} onKeyDown={copyOnKeyDown}>Copy</button><span aria-live="polite">{copyStatus()}</span></div>` : null;
  const fallback = clipboardFallback ?? (nativeButton
...
export function emitSolidLibrary({libraryPath, outputPath} = {}) {
  ...
  for (const model of library.models) {
    ...
    const js = source(model, toggle, nativeInput, fieldControl, modelClipboardCopy), dts = declaration(model, toggle, nativeInput, modelClipboardCopy), variants = model.draftImplementation.semanticVariants ?? [];
