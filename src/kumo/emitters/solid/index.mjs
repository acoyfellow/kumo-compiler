...
function declaration(model) {
  const props = model.props.items.map(item => `  ${JSON.stringify(item.name)}${item.required ? '' : '?'}: ${item.nativeForwarding ? safeType(item.type) : safeType(item.type).includes('callback') ? '(...args: unknown[]) => void' : 'unknown'};`).join('\n');
