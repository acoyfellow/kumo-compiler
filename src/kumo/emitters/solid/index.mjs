...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls, radioGroup, tabsNavigation, menubarNavigation, dialogLayer, inputGroupComposition, sensitiveInput, comboboxCollection, autocompleteCollection, commandPalette, toastLifecycle, datePicker, dateRangePicker, responsiveSidebar, popoverLayer, dropdownMenuLayer, selectCollection) {
  ...
  const dropdownFallback = dropdownMenuLayer ? `<><Show when={dropdownOpen()} children={<span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span>} /><button ref={dropdownTrigger} type="button" tabindex="0" aria-haspopup="menu" aria-expanded={false} data-kumo-component={dropdownOpen() || dropdownTriggerHasIcon() ? "Button" : undefined} class={dropdownOpen() || dropdownTriggerHasIcon() ? ${JSON.stringify(KUMO_OVERLAY_BUTTON_CLASS)} : undefined} onClick={toggleDropdownFromPointer} onKeyDown={dropdownTriggerKeyDown}>{dropdownTriggerHasIcon() ? <>${KUMO_PLUS_ICON_SVG}<span class="contents">{dropdownTriggerText()}</span></> : dropdownTriggerText()}</button><Show when={dropdownOpen()} children={<><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span><div data-base-ui-portal=""><div role="presentation" style="position:absolute;left:0;top:0;transform:translate(-19px,44px)"><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span>{(!dropdownCanonicalOverlay() || props.onOpenChange || props.defaultOpen) ? <div ref={dropdownMenu} role="menu" class=${JSON.stringify(KUMO_DROPDOWN_CONTENT_CLASS)}><For each={dropdownItems()} children={item => <div role="menuitem" tabindex="-1" aria-disabled={item.disabled || undefined} data-kumo-component="DropdownMenu" data-kumo-part="item" data-disabled={item.disabled ? "" : undefined} class=${JSON.stringify(KUMO_DROPDOWN_ITEM_CLASS)} onClick={event => selectDropdownItem(item, event)} onKeyDown={event => dropdownItemKeyDown(event, item)}>{item.text}</div>} /></div> : undefined}<span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span></div></div><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span><span style="clip-path:inset(50%);position:fixed;top:0;left:0"></span><span aria-hidden="true" tabindex="0" style=${JSON.stringify(KUMO_OVERLAY_GUARD_STYLE)}></span></>} /></>` : null;
