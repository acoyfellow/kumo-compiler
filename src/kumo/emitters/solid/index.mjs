...
function source(model, toggle, nativeInput, fieldControl, clipboardCopy, paginationControls, radioGroup, tabsNavigation, menubarNavigation, dialogLayer, inputGroupComposition, sensitiveInput, comboboxCollection, autocompleteCollection, commandPalette, toastLifecycle, datePicker, dateRangePicker, responsiveSidebar, popoverLayer, dropdownMenuLayer, selectCollection) {
  ...
  const nativeInputBare = nativeInputElement ? `<${nativeInputElement} {...native} class={mergeStyles(${JSON.stringify(KUMO_INPUT_CLASS)}, styles.root, props.class)} value={props.${nativeInput?.uncontrolled.prop} as string | number | string[] | undefined} disabled={Boolean(props.disabled)} onInput={nativeInputHandler}></${nativeInputElement}>` : null;
  ...
  const nativeInputFallback = nativeInputBare && ownedControlId ? `props.label != null ? <div><label for=${JSON.stringify(ownedControlId)}>{props.label as JSX.Element}</label><${nativeInputElement} {...native} id=${JSON.stringify(ownedControlId)} class={mergeStyles(${JSON.stringify(KUMO_INPUT_CLASS)}, styles.root, props.class)} value={props.${nativeInput?.uncontrolled.prop} as string | number | string[] | undefined} disabled={Boolean(props.disabled)} onInput={nativeInputHandler}></${nativeInputElement}></div> : ${nativeInputBare}` : nativeInputBare;
  const providedFieldFallback = fieldControl && !fieldControl.ownsControl ? `<div><label for={props.controlId as string ?? "field-control"} class=${JSON.stringify(KUMO_FIELD_LABEL_CLASS)}>{props.label as JSX.Element}</label>{props.children}{(props as Record<string, unknown>).description !== undefined ? <p class=${JSON.stringify(KUMO_FIELD_DESCRIPTION_CLASS)}>{(props as Record<string, unknown>).description as JSX.Element}</p> : null}</div>` : null;
  ...
  const commandPaletteFallback = commandPalette ? `props.text !== undefined ? <span><For each={highlightSegments()} children={segment => segment.mark ? <mark>{segment.text}</mark> : segment.text} /></span> : <Show when={paletteOpen()} fallback={<></>} children={<div data-kumo-component="CommandPalette"><input ref={commandPaletteInput} value={paletteValue()} onInput={commandPaletteOnInput} onKeyDown={commandPaletteKeyDown} /><ul role="listbox"><For each={paletteItems()} children={(item, index) => <li role="option" aria-selected={highlightedPaletteIndex() === index()}>{item.label}</li>} /></ul></div>} />` : null;
   const meterFallback = model.component === 'meter' ? `<div class=${JSON.stringify(KUMO_METER_ROOT_CLASS)} role="meter" aria-valuenow={props.value as number} aria-valuemin={(props.min as number) ?? 0} aria-valuemax={(props.max as number) ?? 100} aria-valuetext={Math.round((((props.value as number) ?? 0) - ((props.min as number) ?? 0)) / (((((props.max as number) ?? 100) - ((props.min as number) ?? 0)) as number) || 1) * 100) + "%"}><div class=${JSON.stringify(KUMO_METER_HEADER_CLASS)}><span class=${JSON.stringify(KUMO_METER_LABEL_CLASS)}>{props.label as JSX.Element}</span>{(props.showValue as boolean) !== false ? <span class=${JSON.stringify(KUMO_METER_VALUE_CLASS)}>{(props.customValue as JSX.Element) ?? ((props.value as number) + "%")}</span> : null}</div><div class=${JSON.stringify(KUMO_METER_TRACK_CLASS)}><div class=${JSON.stringify(KUMO_METER_FILL_CLASS)} style={{ width: (props.value as number) + "%" }} /></div><span role="presentation" style="clip-path:inset(50%);overflow:hidden;white-space:nowrap;border:0;padding:0;width:1px;height:1px;margin:-1px;position:fixed;top:0;left:0">x</span></div>` : null;
