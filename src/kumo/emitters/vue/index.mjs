...
function tooltipPopupSource(model) {
  ...
  return {
    ...
    setup:`const tooltipMounted = ref(false)\nonMounted(() => { tooltipMounted.value = true })\nconst tooltipOpen = computed(() => props.open !== undefined ? Boolean(props.open) : Boolean(props.defaultOpen))\nconst tooltipContent = computed(() => typeof props.content === 'string' || typeof props.content === 'number' ? String(props.content) : fixtureText(props.content))\n`,
    template:`<template v-if="props.asChild"><slot /></template><button v-else class="inline-flex items-center bg-transparent border-none shadow-none p-0 m-0 h-auto min-h-0 leading-[0] cursor-default" type="button" data-base-ui-tooltip-trigger=""><slot /></button><Teleport v-if="tooltipMounted && tooltipOpen" to="body"><div data-base-ui-portal data-base-ui-inert><div data-open="" data-side="top" data-align="center" role="presentation" class="${esc(KUMO_TOOLTIP_POSITIONER_CLASS)}" style="position:absolute;left:0;top:0;transform:translate(0px,-10px)"><div data-open="" data-side="top" data-align="center" tabindex="-1" data-base-ui-focusable="" class="${esc(KUMO_TOOLTIP_POPUP_CLASS)}"><div data-open="" data-side="top" data-align="center" aria-hidden="true" class="${esc(KUMO_TOOLTIP_ARROW_CLASS)}" style="position:absolute;left:12px">${KUMO_TOOLTIP_ARROW_SVG}</div>{{ tooltipContent }}</div></div></div></Teleport>`
  };
}
