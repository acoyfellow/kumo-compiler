...
function node(value, context = {}) {
  ...
  switch (value.kind) {
    ...
    case 'compound': return `<div data-kumo-compound="${esc(value.name)}" :class="styles.root">${Object.entries(value.parts).map(([name, part]) => { const override = compoundPartOverride(context.component, name); return override ? `<${override.tag}${override.className ? ` class="${esc(override.className)}"` : ''}>${child(part)}</${override.tag}>` : `<section data-kumo-part="${esc(name)}">${child(part)}</section>`; }).join('')}</div>`;
...
function compoundPartSource(component, partPath) {
  ...
  const attributes = override ? (override.className ? `class="${esc(override.className)}"` : '') : `data-kumo-part="${esc(partPath)}"`;
