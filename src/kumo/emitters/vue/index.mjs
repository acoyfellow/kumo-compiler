...
function node(value, context = {}) {
  ...
  switch (value.kind) {
    ...
    case 'compound': return `<div data-kumo-compound="${esc(value.name)}" :class="styles.root">${Object.entries(value.parts).map(([name, part]) => { const override = compoundPartOverride(context.component, name); return override ? `<${override.tag}${override.className ? ` class="${esc(override.className)}"` : ''}${override.dataAttr ? ` ${override.dataAttr}=""` : ''}>${child(part)}</${override.tag}>` : `<section data-kumo-part="${esc(name)}">${child(part)}</section>`; }).join('')}</div>`;
    ...
    case 'element': {
      ...
      const childCtx = { ...context, root: false };
      for (const [name, exp] of Object.entries(value.attributes ?? {})) {
...
function compoundPartSource(component, partPath) {
  ...
  const attributes = override ? [override.className ? `class="${esc(override.className)}"` : '', override.dataAttr ? `${override.dataAttr}=""` : ''].filter(Boolean).join(' ') : `data-kumo-part="${esc(partPath)}"`;
...
function dropdownMenuLayerSource(capability) {
  return {
    options:`defineOptions({ inheritAttrs: false })\n`,
    imports:'computed, nextTick, onMounted, ref, useAttrs, useSlots, watch',
...
const triggerRef = ref<HTMLButtonElement | null>(null)
const dropdownPositioner = ref<HTMLElement | null>(null)
const itemRefs = ref<HTMLButtonElement[]>([])
const open = ref(Boolean(props.open ?? props.defaultOpen))
function positionDropdown() {
  const trigger = triggerRef.value; const positioner = dropdownPositioner.value
  if (!trigger || !positioner) return
  const menu = positioner.querySelector('[role="menu"]') as HTMLElement | null
  if (!menu) return
...
onMounted(() => { nextTick(() => requestAnimationFrame(positionDropdown)) })
watch(open, (v) => { if (v) nextTick(() => requestAnimationFrame(positionDropdown)) })
...
`,
    template:`<span v-if="open && canonicalOverlay" aria-hidden="true" tabindex="0" style="${esc(VUE_OVERLAY_GUARD_STYLE)}"></span><button ref="triggerRef" id="kumo-dropdown-trigger" v-bind="$attrs" type="button" tabindex="0" aria-haspopup="menu" :aria-expanded="canonicalOverlay ? false : open" :data-kumo-component="canonicalOverlay && (open || triggerHasIcon) ? 'Button' : 'DropdownMenu'" :data-kumo-part="canonicalOverlay ? undefined : 'trigger'" :data-disabled-skipped="disabledSkipped || undefined" :class="canonicalOverlay && (open || triggerHasIcon) ? '${esc(VUE_OVERLAY_BUTTON_CLASS)}' : undefined" @click="openMenu(false)" @keydown="triggerKey"><template v-if="triggerHasIcon">${svgTemplate(KUMO_PLUS_ICON_SVG)}<span class="contents">{{ partText(triggerPart) }}</span></template><template v-else>{{ partText(triggerPart) }}</template></button><template v-if="open && canonicalOverlay"><span aria-hidden="true" tabindex="0" data-base-ui-focus-guard="" style="${esc(VUE_OVERLAY_GUARD_STYLE)}"></span><span data-type="outside" aria-hidden="true" tabindex="0" data-base-ui-focus-guard="" style="${esc(VUE_OVERLAY_GUARD_STYLE)}"></span><span aria-owns="kumo-dropdown-menu" style="clip-path:inset(50%);position:fixed;top:0;left:0"></span><span data-type="outside" aria-hidden="true" tabindex="0" data-base-ui-focus-guard="" style="${esc(VUE_OVERLAY_GUARD_STYLE)}"></span></template><Teleport v-if="open && canonicalOverlay" to="body"><div id="kumo-dropdown-menu" data-base-ui-portal><div ref="dropdownPositioner" role="presentation" style="position:absolute;left:0;top:0;transform:translate(-19px,44px)"><span aria-hidden="true" tabindex="0" style="${esc(VUE_OVERLAY_GUARD_STYLE)}"></span><div role="menu" aria-labelledby="kumo-dropdown-trigger" class="${esc(VUE_DROPDOWN_CONTENT_CLASS)}"><div v-for="(entry, index) in menuEntries" :key="index" :ref="el => { if (el) itemRefs[index] = el as HTMLButtonElement }" role="menuitem" tabindex="-1" data-kumo-component="DropdownMenu" data-kumo-part="item" :data-disabled="entry.props?.disabled ? '' : undefined" :data-highlighted="activeIndex === index ? '' : undefined" class="${esc(VUE_DROPDOWN_ITEM_CLASS)}" @click="selectItem(entry)" @keydown="entryKey">{{ entry.export === '.Sub' ? partText(fixturePart(entry, '.SubTrigger')) : partText(entry) }}</div></div><span aria-hidden="true" tabindex="0" style="${esc(VUE_OVERLAY_GUARD_STYLE)}"></span></div></div></Teleport><Teleport v-else-if="open && !canonicalOverlay" to="body"><div role="menu"><template v-for="(entry, index) in menuEntries" :key="index"><button v-if="entry.export === '.Item'" :ref="el => { if (el) itemRefs[index] = el as HTMLButtonElement }" type="button" role="menuitem" :tabindex="activeIndex === index ? 0 : -1" :disabled="entry.props?.disabled || undefined" :data-highlighted="activeIndex === index || undefined" @click="selectItem(entry)" @keydown="entryKey">{{ partText(entry) }}</button><button v-else :ref="el => { if (el) itemRefs[index] = el as HTMLButtonElement }" type="button" role="menuitem" :tabindex="activeIndex === index ? 0 : -1" :data-highlighted="activeIndex === index || undefined" aria-haspopup="menu" :aria-expanded="submenuOpen" @keydown="entryKey">{{ partText(fixturePart(entry, '.SubTrigger')) }}</button><div v-if="entry.export === '.Sub' && submenuOpen" role="menu"><button v-for="(nested, nestedIndex) in fixtureChildren(fixturePart(entry, '.SubContent'))" :key="nestedIndex" type="button" role="menuitem" tabindex="-1" @keydown="entryKey">{{ partText(nested) }}</button></div></template></div></Teleport>`
  };
}
...
const VUE_SELECT_TRIGGER_CLASS = 'group flex shrink-0 items-center select-none border-0 shadow-xs focus:outline-none focus-visible:ring-2 focus-visible:ring-kumo-brand cursor-pointer disabled:cursor-not-allowed disabled:text-kumo-subtle h-9 gap-1.5 rounded-lg px-3 text-base bg-kumo-base !text-kumo-default ring not-disabled:hover:bg-kumo-tint disabled:bg-kumo-base/50 disabled:!text-kumo-default/70 ring-kumo-line data-[state=open]:bg-kumo-base justify-between font-normal focus:opacity-100 focus:ring-kumo-focus/50 focus-visible:ring-inset *:in-focus:opacity-100';
...
const selectRootClass = computed(() => '${VUE_SELECT_ROOT_CLASS}')
const selectTriggerClass = computed(() => ['${VUE_SELECT_TRIGGER_CLASS}', (rootAttrs.class as string | undefined) ?? 'w-max'].filter(Boolean).join(' '))
...
`,
    template:`<div data-slot="pagination" class="flex items-center gap-2 w-full"><div aria-live="polite" aria-atomic="true" data-slot="pagination-info" class="grow text-sm text-kumo-subtle">Showing <span class="tabular-nums">{{ infoStart }}-{{ infoEnd }}</span> of <span class="tabular-nums">{{ infoTotal }}</span></div><div data-slot="pagination-controls" class="grow flex flex-col items-end"><nav ref="navRef" :aria-label="props.labels?.navigation ?? 'Pagination'"><div data-slot="input-group" data-focus-mode="individual" class="${esc(VUE_PAGINATION_GROUP_CLASS)}"><button data-kumo-component="Button" :class="['${esc(VUE_PAGINATION_BUTTON_CLASS)}', disabledPage === 1 ? 'cursor-not-allowed opacity-50' : '']" type="button" aria-label="First page" :disabled="disabledPage === 1 || undefined" @click="proposePage(1)"><span class="contents">${VUE_PAGINATION_FIRST_SVG}</span></button><button data-kumo-component="Button" :class="['${esc(VUE_PAGINATION_BUTTON_CLASS)}', disabledPage === 1 ? 'cursor-not-allowed opacity-50' : '']" type="button" aria-label="Previous page" :disabled="disabledPage === 1 || undefined" @click="proposePage(currentPage - 1)"><span class="contents">${VUE_PAGINATION_PREV_SVG}</span></button><input aria-label="Page number" autocomplete="off" class="${esc(VUE_PAGINATION_INPUT_CLASS)}" style="width:50px" :value="editingPage" @input="editingPage = ($event.currentTarget as HTMLInputElement).value" @keydown="enterInput" @blur="commitInput('blur')" /><button data-kumo-component="Button" :class="['${esc(VUE_PAGINATION_BUTTON_CLASS)}', disabledPage === maximumPage ? 'cursor-not-allowed opacity-50' : '']" type="button" aria-label="Next page" :disabled="disabledPage === maximumPage || undefined" @click="proposePage(currentPage + 1)"><span class="contents">${VUE_PAGINATION_NEXT_SVG}</span></button><button data-kumo-component="Button" :class="['${esc(VUE_PAGINATION_BUTTON_CLASS)}', disabledPage === maximumPage ? 'cursor-not-allowed opacity-50' : '']" type="button" aria-label="Last page" :disabled="disabledPage === maximumPage || undefined" @click="proposePage(maximumPage)"><span class="contents">${VUE_PAGINATION_LAST_SVG}</span></button></div></nav></div></div>`
  };
}
...
function emitComponent(model, library) {
  ...
  // against vue's own SSR renderer). Backfill those via a get-proxy over the
  // reactive props so the default is honored while reactivity is preserved.
  const hyphenatedDefaults = Object.fromEntries(Object.entries(defaults).filter(([name]) => name.includes('-')));
  const hyphenatedDefaultsSnippet = Object.keys(hyphenatedDefaults).length
    ? `const __hyphenatedDefaults: Record<string, unknown> = ${JSON.stringify(hyphenatedDefaults)}\nconst props = new Proxy(__declaredProps as Record<string, unknown>, { get(target, key: string) { const value = Reflect.get(target, key); return value === undefined && key in __hyphenatedDefaults ? __hyphenatedDefaults[key] : value } }) as typeof __declaredProps\n`
    : '';
  const declaredPropsName = Object.keys(hyphenatedDefaults).length ? '__declaredProps' : 'props';
  ...
  const loweredTooltipPopup = tooltipPopupSource(model);
  ...
  if (dropdownMenuLayer) { declaredProps.set('open',{name:'open',required:false,type:'boolean'}); declaredProps.set('defaultOpen',{name:'defaultOpen',required:false,type:'boolean'}); declaredProps.set('modal',{name:'modal',required:false,type:'boolean'}); declaredProps.set('onOpenChange',{name:'onOpenChange',required:false,type:'unknown'}); declaredProps.set('onSelect',{name:'onSelect',required:false,type:'unknown'}); }
  ...
  const predicates = variants.map(v => v.when.map(x => semanticPredicate(x.kind === 'prop-equals' && x.name !== 'children' ? {...x,name:vuePropName(x.name)} : x,{props:'semanticValues',fixture:'fixture',content:'props.semanticContent',equal:'semanticEqual'})).join(' && ') || 'true');
   ...
   // Label: same composition as Svelte/Solid, verified against @cloudflare/kumo
   ...
   // state).
   const labelFallback = label ? `<label class="${esc(KUMO_LABEL_ROOT_CLASS)}"><slot /><span v-if="props.showOptional" class="${esc(KUMO_LABEL_OPTIONAL_CLASS)}">(optional)</span><button v-if="props.tooltip !== undefined" data-kumo-component="Button" class="${esc(KUMO_LABEL_INFO_BUTTON_CLASS)}" type="button" aria-label="More information" data-base-ui-tooltip-trigger><span class="contents">${KUMO_LABEL_INFO_ICON_SVG}</span></button></label>` : null;
   const surface = model.component === 'surface';
   const surfaceFallback = surface ? `<div :data-surface-color="props.color ?? 'primary'" data-deprecated="surface" :class="['bg-kumo-base', 'shadow-xs', 'ring', 'ring-kumo-line', props.className ? undefined : 'overflow-visible', props.className ? undefined : 'rounded-none', props.className]"><slot /></div>` : null;
   const grid = model.component === 'grid';
   const layerCard = model.component === 'layer-card';
   // LayerCard root: detect Primary/Secondary sub-parts (data-kumo-layer-part)
   ...
   const gridGapExpr = Object.entries(KUMO_GRID_GAP_CLASSES).map(([k,v]) => `(props.gap ?? 'base') === '${k}' ? '${v}'`).join(' : ') + ` : '${KUMO_GRID_GAP_CLASSES.base}'`;
   const gridFallback = grid ? `<div :class="['grid', ${directive(gridVariantExpr)}, ${directive(gridGapExpr)}, (props as any).className]"><slot /></div>` : null;
   const layerCardSetup = layerCard ? `const layerCardRoot = ref<HTMLElement | null>(null)\nconst layerCardMode = ref<'plain'|'layered'>('plain')\nconst syncLayerCardMode = () => { if (layerCardRoot.value) layerCardMode.value = layerCardRoot.value.querySelector('[data-kumo-layer-part]') ? 'layered' : 'plain' }\nonMounted(syncLayerCardMode)\nonUpdated(syncLayerCardMode)\n` : '';
   const meterLabelId = meter ? `kumo-${sha(model.modelDigest).slice(0, 12)}-meter-label` : null;
   const meterFallback = meter ? `<div class="${esc(KUMO_METER_ROOT_CLASS)}" role="meter" :aria-valuenow="props.value" :aria-valuemin="props.min ?? 0" :aria-valuemax="props.max ?? 100" :aria-valuetext="Math.round(((props.value ?? 0) - (props.min ?? 0)) / (((props.max ?? 100) - (props.min ?? 0)) || 1) * 100) + '%'" aria-labelledby="${esc(meterLabelId)}"><div class="${esc(KUMO_METER_HEADER_CLASS)}"><span id="${esc(meterLabelId)}" role="presentation" class="${esc(KUMO_METER_LABEL_CLASS)}">{{ props.label }}</span><span v-if="props.showValue !== false" class="${esc(KUMO_METER_VALUE_CLASS)}">{{ props.customValue ?? (props.value + '%') }}</span></div><div class="${esc(KUMO_METER_TRACK_CLASS)}"><div class="${esc(KUMO_METER_FILL_CLASS)}" :style="{ 'inset-inline-start': '0', height: 'inherit', width: props.value + '%' }"></div></div><span role="presentation" style="clip-path:inset(50%);overflow:hidden;white-space:nowrap;border:0;padding:0;width:1px;height:1px;margin:-1px;position:fixed;top:0;left:0">x</span></div>` : null;
   const semantic = (loweredTooltipPopup || select || datePicker || dateRangePicker || toastLifecycle || responsiveSidebar || nativeInput || clipboardCopy || pagination || radioGroup || tabsNavigation || menubarNavigation || dialogLayer || popoverLayer || dropdownMenuLayer || inputGroup || sensitiveInput || combobox || autocomplete || commandPalette || tableOfContents || meter || banner || collapsible || layerCard) ? '' : variants.map((v,i) => `<template ${i?'v-else-if':'v-if'}="${directive(predicates[i])}">${semanticNode(v.tree)}</template>`).join('');
  ...
  const visualSimpleFallback = visualSimple ? (model.component === 'badge'
    ...
    : model.component === 'text'
    ? `<p v-bind="$attrs" :class="['text-kumo-default', ({xs:'text-xs',sm:'text-sm',base:'text-base',lg:'text-lg'} as Record<string,string>)[props.size ?? 'base'] ?? 'text-base', props.bold ? 'font-medium' : '']"><slot /></p>`
    : `<${visualSimple.root.tag} v-bind="$attrs" class="${visualSimple.root.className}"><slot /></${visualSimple.root.tag}>`) : null;
  const bannerFallback = banner ? `<div :class="${directive(bannerClassExpression('props.variant', 'root'))}"><template v-if="props.title !== undefined || props.description !== undefined || props.action !== undefined"><span v-if="$slots.icon || props.icon" :class="${directive(`[${JSON.stringify(KUMO_BANNER_STRUCTURED_ICON_CLASS)}, ${bannerClassExpression('props.variant', 'text')}]`)}"><slot name="icon"><component :is="props.icon" /></slot></span><div :class="['flex min-w-0 flex-1 items-center justify-between gap-3', props.title === undefined ? 'pt-px' : '']"><div class="flex flex-col gap-0.5"><p v-if="props.title !== undefined" class="font-medium leading-snug">{{ props.title }}</p><div v-if="props.description !== undefined || $slots.description" class="text-sm leading-snug"><p><slot name="description">{{ props.description }}</slot></p></div></div><div v-if="props.action !== undefined || $slots.action" class="flex shrink-0 items-center gap-2"><slot name="action">{{ props.action }}</slot></div></div></template><template v-else><span v-if="$slots.icon || props.icon" :class="${directive(`[${JSON.stringify(KUMO_BANNER_SIMPLE_ICON_CLASS)}, ${bannerClassExpression('props.variant', 'text')}]`)}"><slot name="icon"><component :is="props.icon" /></slot></span><p><slot>{{ props.text }}</slot></p></template></div>` : null;
  const collapsibleFallback = collapsible ? `<div :data-open="props.open || props.defaultOpen ? '' : undefined" :data-closed="props.open || props.defaultOpen ? undefined : ''" :title="props.title" :label="props.label"><slot /></div>` : null;
  const fallback = loweredTooltipPopup?.template ?? labelFallback ?? surfaceFallback ?? gridFallback ?? layerCardFallback ?? visualSimpleFallback ?? bannerFallback ?? collapsibleFallback ?? loweredTableOfContents?.template ?? loweredSelect?.template ?? loweredDatePicker?.template ?? loweredDateRangePicker?.template ?? loweredToastLifecycle?.template ?? loweredResponsiveSidebar?.template ?? loweredCommandPalette?.template ?? loweredAutocomplete?.template ?? loweredCombobox?.template ?? loweredSensitiveInput?.template ?? loweredInputGroup?.template ?? loweredDropdownMenuLayer?.template ?? loweredPopoverLayer?.template ?? loweredDialogLayer?.template ?? loweredMenubarNavigation?.template ?? loweredTabsNavigation?.template ?? loweredRadioGroup?.template ?? loweredPagination?.template ?? loweredClipboardCopy?.template ?? loweredToggle?.template ?? loweredNativeInput?.template ?? meterFallback ?? (nativeButton
    ? `<button v-bind="Object.assign({}, $attrs, mergeTriggerAttributes)" :class="${directive(`kumoCxBtn(${nativeButtonVariantExpression(nativeButton, 'props.variant')})`)}" :style="${directive(nativeButtonEmphasisStyleExpression(nativeButton.emphasis, 'props.variant'))}" :type="($attrs.type as any) ?? 'button'" :disabled="props.disabled || props.loading"><template v-if="${directive(nativeButtonEmphasisCondition(nativeButton.emphasis, 'props.variant'))}"><span aria-hidden="true" class="${esc(nativeButton.emphasis.overlayClass)}"></span><span class="${esc(nativeButton.emphasis.wrapperClass)}"><template v-if="props.loading">${vueButtonSpinner("props.size === 'lg' ? 16 : 14")}</template><slot v-else-if="$slots.icon" name="icon" /><slot /></span></template><template v-else><template v-if="props.loading">${vueButtonSpinner("props.size === 'lg' ? 16 : 14")}</template><slot v-else-if="$slots.icon" name="icon" /><span v-if="$slots.default" class="contents"><slot /></span></template></button>`
    : node(implementation.componentRoot, {component:model.component, root:true}));
  // True only when the fallback is the bare component-root node() path (not a
  // lowered/native-button template). That path merges the consumer class into
  // the root via kumoCx(kumoRootAttrs.class), so it must own attribute
  // inheritance (inheritAttrs:false) to avoid Vue ALSO auto-appending the raw
  // class (which would reintroduce the un-merged w-full/text-base).
  const bareRoot = !loweredTooltipPopup && !labelFallback && !surfaceFallback && !visualSimpleFallback && !bannerFallback && !collapsibleFallback && !loweredTableOfContents && !loweredSelect && !loweredDatePicker && !loweredDateRangePicker && !loweredToastLifecycle && !loweredResponsiveSidebar && !loweredCommandPalette && !loweredAutocomplete && !loweredCombobox && !loweredSensitiveInput && !loweredInputGroup && !loweredDropdownMenuLayer && !loweredPopoverLayer && !loweredDialogLayer && !loweredMenubarNavigation && !loweredTabsNavigation && !loweredRadioGroup && !loweredPagination && !loweredClipboardCopy && !loweredToggle && !loweredNativeInput && !meterFallback && !nativeButton;
...
export function generateVueLibrary(output = path.join(root, 'generated/libraries/vue')) {
  ...
  const models = expandNamedExportModels(library.models);
