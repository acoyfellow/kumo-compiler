...
function toggleSource({state,native}) {
  ...
  const rootAttrs = native.root === 'button' ? ` type="button"` : ` tabindex="0"`;
  const keyHandler = native.root === 'button' ? '' : ` @keydown.space.prevent="activate"`;
  const aria = indeterminate ? `(props.${indeterminate.prop} ? 'mixed' : currentChecked)` : 'currentChecked';
  return {
    options:`defineOptions({ inheritAttrs: false })\n`,
    imports:'computed, getCurrentInstance, ref, useAttrs, useSlots',
    setup:`const emit = defineEmits<{ checkedChange: [checked: boolean] }>()\nconst instance = getCurrentInstance()\nconst controlled = Object.prototype.hasOwnProperty.call(instance?.vnode.props ?? {}, ${JSON.stringify(state.controlledProp)})\nconst internalChecked = ref(props.${state.defaultProp} ?? ${JSON.stringify(state.initial)})\nconst currentChecked = computed(() => controlled ? props.${state.controlledProp} : internalChecked.value)\nfunction activate(event: Event) {\n  if (props.${state.disabled.prop}) return\n  const next = ${indeterminate ? `props.${indeterminate.prop} ? ${JSON.stringify(indeterminate.activationResult)} : ` : ''}!currentChecked.value\n  if (!controlled) internalChecked.value = next\n  props.onCheckedChange?.(next, event)\n  emit('checkedChange', next)\n}\n`,
    template:`<${native.root} v-bind="$attrs"${rootAttrs} role="${role}" :aria-checked="${aria}" :aria-disabled="props.${state.disabled.prop} || undefined" :disabled="props.${state.disabled.prop} || undefined" @click="activate"${keyHandler}><slot />{{ props.label }}</${native.root}>`
  };
}
