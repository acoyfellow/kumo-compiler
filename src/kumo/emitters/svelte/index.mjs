...
function component(model,capabilities){
 ...
 const tocFallback=tableOfContents?`<nav aria-label={String(tocFixture?.props?.['aria-label'] ?? 'Table of contents')}>{#if tocTitle}<p>{tocTitle}</p>{/if}<ul>{#each tocItems as item (item.href)}{#if item.group}<a href={item.href} aria-current={item.active ? 'location' : undefined}>{item.label}</a>{:else}<li><a href={item.href} aria-current={item.active ? 'location' : undefined}>{item.label}</a></li>{/if}{/each}</ul></nav>`:null;
  ...
  const meterFallback=meter?`<div class=${q(KUMO_METER_ROOT_CLASS)} role="meter" aria-valuenow={value} aria-valuemin={min ?? 0} aria-valuemax={max ?? 100}><div class=${q(KUMO_METER_HEADER_CLASS)}><span class=${q(KUMO_METER_LABEL_CLASS)}>{label}</span>{#if showValue !== false}<span class=${q(KUMO_METER_VALUE_CLASS)}>{customValue ?? (value + "%")}</span>{/if}</div><div class=${q(KUMO_METER_TRACK_CLASS)}><div class=${q(KUMO_METER_FILL_CLASS)} style="width: {value}%"></div></div><span role="presentation" class="sr-only">{customValue ?? (value + "%")}</span></div>`:null;
 ...
 const inputFallback=nativeInput&&fieldControl?.ownsControl?`{#if label !== undefined}<div><label for={controlId}>{label}</label>${labelledInput}</div>{:else}${bareInput}{/if}`:bareInput;
  const fieldFallback=fieldControl&&!fieldControl.ownsControl?`<div {...rest}><label for={controlId} class=${q(KUMO_FIELD_LABEL_CLASS)}><span class="inline-flex items-center gap-1">{label}</span></label>{#if children}{@render children()}{/if}{#if description !== undefined}<p class=${q(KUMO_FIELD_DESCRIPTION_CLASS)}>{description}</p>{/if}</div>`:null;
 ...
 const comboboxFallback=combobox?`<input bind:this={comboboxInput} role="combobox" aria-expanded={comboboxOpen} placeholder={comboboxFixture.placeholder} value={comboboxValue} onclick={openCombobox} onkeydown={handleComboboxKey}>{#if comboboxOpen}<ul role="listbox">{#each comboboxFixture.items as item, index (item.value)}<li role="option" data-value={item.value} aria-selected={highlightedIndex === index}>{item.label}</li>{/each}</ul>{/if}`:null;
