...
function component(model,capabilities){
 ...
 const inputFallback=nativeInput&&fieldControl?.ownsControl?`{#if label !== undefined}<div><label for={controlId}>{label}</label>${labelledInput}</div>{:else}${bareInput}{/if}`:bareInput;
  const fieldFallback=fieldControl&&!fieldControl.ownsControl?`<div {...rest}><label for={controlId} class=${q(KUMO_FIELD_LABEL_CLASS)}>{label}</label>{#if children}{@render children()}{/if}{#if description !== undefined}<p class=${q(KUMO_FIELD_DESCRIPTION_CLASS)}>{description}</p>{/if}</div>`:null;
