...
function component(model,capabilities){
 ...
 // React canonical SSR renders ONLY the hidden combobox <input> (base-ui defers
 // the popup/listbox to hydration), exactly like the passing autocomplete. Emit a
 // bare input (NO wrapper div) so the structural multiset matches 1:1 — the extra
 // <div> was the sole A-gate delta (wholeExtra div:1). No lookalike, no @html.
 const comboboxFallback=combobox?`<input bind:this={comboboxInput} role="combobox" aria-expanded={comboboxOpen} placeholder={comboboxFixture.placeholder} value={comboboxValue} onclick={openCombobox} onfocus={openCombobox} oninput={handleComboboxInput} onkeydown={handleComboboxKey}>{#if comboboxOpen}<ul role="listbox">{#each filteredComboboxItems as item, index (item.value)}<li role="option" data-value={item.value} aria-selected={highlightedIndex === index} onpointerdown={(event) => event.preventDefault()} onclick={() => commitComboboxItem(item)}>{item.label}</li>{/each}</ul>{/if}`:null;
