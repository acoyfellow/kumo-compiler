...
function component(model,capabilities){
 ...
 const tocFallback=tableOfContents?`<nav aria-label={String(tocFixture?.props?.['aria-label'] ?? 'Table of contents')}>{#if tocTitle}<p>{tocTitle}</p>{/if}<ul>{#each tocItems as item (item.href)}{#if item.group}<a href={item.href} aria-current={item.active ? 'location' : undefined}>{item.label}</a>{:else}<li><a href={item.href} aria-current={item.active ? 'location' : undefined}>{item.label}</a></li>{/if}{/each}</ul></nav>`:null;
  ...
  const checkboxIndicator=isCheckbox?`<span class=${q(KUMO_CHECKBOX_INDICATOR_CLASS)} data-checked={currentChecked ? "" : undefined} data-unchecked={currentChecked ? undefined : ""} data-indeterminate={(${indeterminateValue}) ? "" : undefined}>{@html (${indeterminateValue}) ? ${q(KUMO_CHECKBOX_MINUS_SVG)} : ${q(KUMO_CHECKBOX_CHECK_SVG)}}</span>`:'';
 ...
 const clipboardFallback=clipboard?`<div {...rest} class=${q(KUMO_CLIPBOARD_ROOT_CLASS)}><span class=${q(KUMO_CLIPBOARD_TEXT_CLASS)}>{text}</span><button data-kumo-component="Button" type="button" class=${q(KUMO_CLIPBOARD_BUTTON_CLASS)} aria-label="Copy to clipboard" onclickcapture={copyText}><span class="contents"><span class=${q(KUMO_CLIPBOARD_CHECK_SPAN_CLASS)}>{@html ${q(KUMO_CLIPBOARD_CHECK_SVG)}}</span><span class=${q(KUMO_CLIPBOARD_COPY_SPAN_CLASS)}>{@html ${q(KUMO_CLIPBOARD_COPY_SVG)}}</span></span></button><span class="sr-only" aria-live="polite">{copyStatus}</span></div>`:null;
