...
function component(model,capabilities){
 const {nativeButton,fieldComposition,clipboardCopy,paginationControls,radioGroup,tabsNavigation,menubarNavigation,dialogLayer,inputGroupComposition,sensitiveInput,comboboxCollection,autocompleteCollection,commandPalette,toastLifecycle,dateRange,responsiveSidebar}=capabilities;
 ...
 const semantic=(nativeInput||clipboard||pagination||radio||tabs||menubar||dialog||inputGroup||sensitive||combobox||autocomplete||palette||toasty||datePicker||dateRangePicker||sidebar)?'':[...variants].sort((a,b)=>b.when.length-a.when.length).map((variant,index)=>`${index?'{:else if':'{#if'} ${variant.when.map(predicate).join(' && ')||'true'}}\n${node(variant.tree,{},1)}`).join('\n');
