...
function component(model,capabilities){
 const {nativeButton,fieldComposition,clipboardCopy}=capabilities;
 ...
 const clipboardFallback=clipboard?`<div {...rest}>{text}<button type="button" onclickcapture={copyText} onkeydowncapture={copyTextOnEnter}>Copy</button><span aria-live="polite">{copyStatus}</span></div>`:null;
 const fallback=hasNativeButton?`<button {...rest} type={type} disabled={Boolean(disabled || loading)} {onclick}>{#if loading}<svg aria-hidden="true"></svg>{/if}{#if children}{@render children()}{/if}</button>`:toggleFallback??inputFallback??fieldFallback??clipboardFallback??node(impl.componentRoot);
