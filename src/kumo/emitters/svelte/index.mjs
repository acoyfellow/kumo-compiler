...
function component(model,capabilities){
 ...
 const nativeDeclarations=hasNativeButton?['type = '+q(nativeButton.type.default),'onclick = undefined']:toggle?['defaultChecked = '+q(toggle.controlled.initial)]:nativeInput?['defaultValue = undefined','disabled = false','onInput = undefined','onFocus = undefined']:[];
 ...
 const inputFallback=nativeInput?nativeInput.native.root==='input'?`<input {...rest} value={defaultValue} disabled={Boolean(disabled)} oninput={handleNativeInput} onfocus={handleNativeFocus}>`:`<textarea {...rest} value={defaultValue} disabled={Boolean(disabled)} oninput={handleNativeInput} onfocus={handleNativeFocus}></textarea>`:null;
 const fallback=hasNativeButton?`<button {...rest} type={type} disabled={Boolean(disabled || loading)} {onclick}>{#if loading}<svg aria-hidden="true"></svg>{/if}{#if children}{@render children()}{/if}</button>`:toggleFallback??inputFallback??node(impl.componentRoot);
