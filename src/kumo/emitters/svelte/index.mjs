...
function component(model,capabilities){
 const {nativeButton,fieldComposition}=capabilities;
 ...
 const fallback=hasNativeButton?`<button {...rest} type={type} disabled={Boolean(disabled || loading)} {onclick}>{#if loading}<svg aria-hidden="true"></svg>{/if}{#if children}{@render children()}{/if}</button>`:toggleFallback??inputFallback??fieldFallback??node(impl.componentRoot);
