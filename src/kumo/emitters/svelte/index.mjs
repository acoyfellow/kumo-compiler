...
function component(model,capabilities){
 const {nativeButton,fieldComposition,clipboardCopy,paginationControls,radioGroup}=capabilities;
 ...
 const semantic=(nativeInput||clipboard||pagination||radio)?'':[...variants].sort((a,b)=>b.when.length-a.when.length).map((variant,index)=>`${index?'{:else if':'{#if'} ${variant.when.map(predicate).join(' && ')||'true'}}\n${node(variant.tree,{},1)}`).join('\n');
 ...
 const radioFallback=radio?`<div bind:this={radioRoot} role="radiogroup" aria-label={radioFixture.legend} tabindex="-1">{#each radioFixture.items as item, index (item.value)}<button type="button" role="radio" aria-checked={selectedRadioValue === item.value} aria-label={item.label} disabled={Boolean(radioFixture.disabled || item.disabled)} onclickcapture={() => selectRadio(item)} onkeydowncapture={(event) => selectNextRadio(event, index)}>{item.label}</button>{/each}</div>`:null;
 const fallback=hasNativeButton?`<button {...rest} type={type} disabled={Boolean(disabled || loading)} {onclick}>{#if loading}<svg aria-hidden="true"></svg>{/if}{#if children}{@render children()}{/if}</button>`:toggleFallback??inputFallback??fieldFallback??clipboardFallback??paginationFallback??radioFallback??node(impl.componentRoot);
