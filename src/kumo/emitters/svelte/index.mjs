...
function component(model,capabilities){
 const {nativeButton}=capabilities;
 ...
 const nativeDeclarations=hasNativeButton?['type = '+q(nativeButton.type.default),'onclick = undefined']:toggle?['defaultChecked = '+q(toggle.controlled.initial)]:[];
 ...
 const indeterminateValue=toggle?.controlled.indeterminate?safeName(toggle.controlled.indeterminate.prop):'false';
 const toggleFallback=toggle?`<button {...rest} type="button" role=${q(toggleRole)} aria-checked={${indeterminateValue} ? "mixed" : currentChecked} disabled={disabled} onclick={activateToggle}>{#if label}{label}{/if}</button>`:null;
 const fallback=hasNativeButton?`<button {...rest} type={type} disabled={Boolean(disabled || loading)} {onclick}>{#if loading}<svg aria-hidden="true"></svg>{/if}{#if children}{@render children()}{/if}</button>`:toggleFallback??node(impl.componentRoot);
