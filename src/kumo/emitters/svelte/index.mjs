...
function component(model,capabilities){
 ...
 const tocFallback=tableOfContents?`<nav aria-label={String(tocFixture?.props?.['aria-label'] ?? 'Table of contents')}>{#if tocTitle}<p>{tocTitle}</p>{/if}{#if tocItems.length}<ul>{#each tocItems as item (item.href)}{#if item.group}<a href={item.href} aria-current={item.active ? 'location' : undefined}>{item.label}</a>{:else}<li><a href={item.href} aria-current={item.active ? 'location' : undefined}>{item.label}</a></li>{/if}{/each}</ul>{/if}</nav>`:null;
  ...
  const bareInput=nativeInput?nativeInput.native.root==='input'?`<input class={${inputErrorClassExpr}} {...rest} value={value !== undefined ? value : defaultValue} disabled={Boolean(disabled)} oninput={handleNativeInput} onfocus={handleNativeFocus}>`:`<textarea class={${inputAreaErrorClassExpr}} {...rest} value={value !== undefined ? value : defaultValue} disabled={Boolean(disabled)} oninput={handleNativeInput} onfocus={handleNativeFocus}></textarea>`:null;
  const labelledInput=nativeInput&&fieldControl?.ownsControl?nativeInput.native.root==='input'?`<input class={${inputErrorClassExpr}} {...rest} id={controlId} value={value !== undefined ? value : defaultValue} disabled={Boolean(disabled)} oninput={handleNativeInput} onfocus={handleNativeFocus}>`:`<textarea class={${inputAreaErrorClassExpr}} {...rest} id={controlId} value={value !== undefined ? value : defaultValue} disabled={Boolean(disabled)} oninput={handleNativeInput} onfocus={handleNativeFocus}></textarea>`:null;
