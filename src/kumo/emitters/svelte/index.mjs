...
function component(model,nativeButton){
 ...
 const nativeDeclarations=hasNativeButton?['type = '+q(nativeButton.type.default),'onclick = undefined']:[];
 ...
 const fallback=hasNativeButton?`<button {...rest} type={type} disabled={Boolean(disabled || loading)} {onclick}>{#if loading}<svg aria-hidden="true"></svg>{/if}{#if children}{@render children()}{/if}</button>`:node(impl.componentRoot);
...
export function emitSvelteLibrary({output=path.join(projectRoot,'generated/libraries/svelte')}={}){
 const {models,nativeButton}=loadLibrary(path.join(projectRoot,'src/kumo/library'));fs.rmSync(output,{recursive:true,force:true});fs.mkdirSync(path.join(output,'components'),{recursive:true});
 const files=[],compoundExports=[];
 for(const model of models){
  const file=`components/${model.component}.svelte`,source=component(model,nativeButton);fs.writeFileSync(path.join(output,file),source);files.push({component:model.component,file:`./${file}`,subpath:model.public.subpath,modelDigest:model.modelDigest,contentBindingDigest:model.contentBindings.capabilityDigest,semanticVariants:(model.draftImplementation.semanticVariants??[]).map(({id,expectationDigest})=>({id,expectationDigest})),unresolvedSemanticOperations:model.unresolvedSemanticOperations??[],sha256:crypto.createHash('sha256').update(source).digest('hex'),exports:model.public.exports});
