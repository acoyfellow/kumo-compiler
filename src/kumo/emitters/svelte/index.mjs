...
function component(model,capabilities){
 const {nativeButton,fieldComposition,clipboardCopy,paginationControls,radioGroup,tabsNavigation,menubarNavigation,dialogLayer,inputGroupComposition,sensitiveInput}=capabilities;
 ...
 const semantic=(nativeInput||clipboard||pagination||radio||tabs||menubar||dialog||inputGroup||sensitive)?'':[...variants].sort((a,b)=>b.when.length-a.when.length).map((variant,index)=>`${index?'{:else if':'{#if'} ${variant.when.map(predicate).join(' && ')||'true'}}\n${node(variant.tree,{},1)}`).join('\n');
