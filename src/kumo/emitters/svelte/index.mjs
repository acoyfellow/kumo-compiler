...
export function emitSvelteLibrary({output=path.join(projectRoot,'generated/libraries/svelte')}={}){
 ...
 fs.writeFileSync(path.join(output,'index.js'),rootLines.join('\n')+'\n');fs.writeFileSync(path.join(output,'index.d.ts'),rootLines.join('\n')+'\n');
