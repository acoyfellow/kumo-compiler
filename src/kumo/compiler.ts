...
for(const ir of catalog.filter(x=>x.root)){
 ...
 const css=await readFile(ir.family==='form'?'public/form.css':ir.family==='native-control'?'public/native-control.css':ir.family==='navigation'?'public/navigation.css':'public/styles.css','utf8');
