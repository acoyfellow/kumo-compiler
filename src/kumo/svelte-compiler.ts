...
function script(ir:ComponentIR):string {const b=ir.behavior;if(!b)return '';
 ...
 if(b.kind==='roving')code=`const labels=${JSON.stringify(b.labels)};let active=$state(labels[0]);function activate(x){active=x}function navigate(e,x){let i=labels.indexOf(x);if(e.key==='ArrowRight')i=(i+1)%labels.length;else if(e.key==='ArrowLeft')i=(i-1+labels.length)%labels.length;else if(e.key==='Home')i=0;else if(e.key==='End')i=labels.length-1;else if(${b.selection==='activation'}&&(e.key==='Enter'||e.key===' ')){activate(x);return}else return;e.preventDefault();active=labels[i];queueMicrotask(()=>e.currentTarget.parentElement.querySelectorAll('[role=${b.itemRole}]')[i]?.focus())}`;
