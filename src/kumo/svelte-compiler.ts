...
function template(node:Node,ir:ComponentIR):string {
 ...
 if(b?.kind==='selection'){
  ...
  if(b.mode==='autocomplete'){if(a.id==='search')extra+=' bind:value={query} aria-expanded={open} onfocus={()=>open=true} onkeydown={key}';if(a.role==='listbox'){before='{#if open}';after='{/if}'}if(a.role==='option'){before=`{#if matches(${JSON.stringify(l)})}`;after='{/if}';extra+=` aria-selected={value===${JSON.stringify(l)}} onclick={()=>choose(${JSON.stringify(l)})} onkeydown={(event)=>chooseKey(event,${JSON.stringify(l)})}`;}}
  if(b.mode==='combobox'||b.mode==='menu'){if(cls.includes(b.mode==='menu'?'menu-button':'combo'))extra+=' aria-expanded={open} onclick={()=>open=!open} onkeydown={key}';if(a.role==='listbox'||a.role==='menu'){before='{#if open}';after='{/if}'}if(a.role==='option')extra+=` aria-selected={value===${JSON.stringify(l)}} onclick={()=>choose(${JSON.stringify(l)})} onkeydown={(event)=>chooseKey(event,${JSON.stringify(l)})}`;else if(a.role==='menuitem')extra+=` onclick={()=>choose(${JSON.stringify(l)})}`;}
  if(b.mode==='command'){if(cls==='command')extra+=' onclick={()=>open=true}';if(a.role==='dialog'){before='{#if open}';after='{/if}';extra+=' aria-modal="true" tabindex="-1" onkeydown={key}'}if(node.tag==='input')extra+=' bind:value={query}';if(a.role==='option'){before=`{#if matches(${JSON.stringify(l)})}`;after='{/if}';extra+=' aria-selected={false}';}}
 ...
 if(b?.kind==='select'){if(a.role==='combobox')extra+=' aria-expanded={open} onclick={()=>open=!open} onkeydown={key}';if(a.role==='listbox'){before='{#if open}';after='{/if}'}if(a.role==='option'){const l=label(node);extra+=` aria-selected={value===${JSON.stringify(l)}} onclick={()=>choose(${JSON.stringify(l)})} onkeydown={(event)=>chooseKey(event,${JSON.stringify(l)})}`;}}
 ...
 const rendered={...a};for(const key of ['type','checked','tabindex','aria-selected','aria-current','aria-expanded','aria-pressed','aria-modal'])if(extra.includes(` ${key}=`)||(key==='checked'&&extra.includes('bind:checked'))||(key==='type'&&extra.includes(' type={')))delete rendered[key];if(extra.includes('bind:value'))delete rendered.value;if(b?.kind==='selection'&&b.mode==='toast'&&node.tag==='output')delete rendered.role;
...
function script(ir:ComponentIR):string {const b=ir.behavior;if(!b)return '';
 ...
 if(b.kind==='roving')code=`const labels=${JSON.stringify(b.labels)};let active=$state(labels[0]);function activate(x){active=x}function navigate(e,x){const target=e.currentTarget;let i=labels.indexOf(x);if(e.key==='ArrowRight')i=(i+1)%labels.length;else if(e.key==='ArrowLeft')i=(i-1+labels.length)%labels.length;else if(e.key==='Home')i=0;else if(e.key==='End')i=labels.length-1;else if(${b.selection==='activation'}&&(e.key==='Enter'||e.key===' ')){activate(x);return}else return;e.preventDefault();const group=target?.parentElement;active=labels[i];queueMicrotask(()=>group?.querySelectorAll('[role=${b.itemRole}]')[i]?.focus())}`;
 if(b.kind==='current-link')code=`let current=$state(${JSON.stringify(b.labels[0])});`;
 if(b.kind==='selection'){const init=JSON.stringify(b.initial??'');code=`let value=$state(${init}),query=$state(''),open=$state(false),message=$state(${b.mode==='toast'?init:"'Ready'"}),page=$state(${b.mode==='pagination'?init:'1'});let range=${b.mode==='date-range'?init:"['','']"},start=$state(range[0]),end=$state(range[1]);const matches=x=>x.toLowerCase().includes(query.toLowerCase());function choose(x){value=x;query=x;open=false}function chooseKey(e,x){if(e.key==='Enter'||e.key===' '){e.preventDefault();choose(x)}}function key(e){if(e.key==='Escape')open=false;else if(e.key==='ArrowDown'||e.key==='ArrowUp'){e.preventDefault();open=true}}`;}
 if(b.kind==='select')code=`let open=$state(false),value=$state('');function choose(x){value=x;open=false}function chooseKey(e,x){if(e.key==='Enter'||e.key===' '){e.preventDefault();choose(x)}}function key(e){if(e.key==='Escape')open=false;else if(['ArrowDown','ArrowUp','Enter',' '].includes(e.key)){e.preventDefault();open=true}}`;
